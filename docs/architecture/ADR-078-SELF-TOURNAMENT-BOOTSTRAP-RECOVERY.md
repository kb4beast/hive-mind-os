...
9. The Windows deep-clone regression removes its clone explicitly through an extended path, clears Git's read-only pack attributes, and retries only the documented transient non-empty-directory condition. It uses Python's 3.12+ `onexc` callback and the declared Python 3.11 minimum's compatible `onerror` callback. Assertions execute before cleanup; the teardown change does not weaken the tested clone invariants.
