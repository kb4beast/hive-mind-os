...
class IsolationAttestation:
    ...
    def from_dict(cls, value):
        ...
        if set(value) != required:
            raise IsolationError("closed attestation schema")
        return cls(
            **{
                **value,
                "probe_result": ProbeResult(value["probe_result"]),
                "writable_mounts": tuple(value["writable_mounts"]),
            }
        )
...
class IsolationBackend(Protocol):
    ...
    def execute(
        self, argv: Sequence[str], *, attestation: IsolationAttestation
    ) -> ExecutionResult: ...
    def teardown(self) -> str: ...


class UnavailableIsolationBackend:
    def __init__(self, reason="OCI broker unavailable"):
        self.reason = reason
    ...
    def capabilities(self):
        return IsolationAttestation(
            ...
            (),
            {},
            canonical_digest(self.reason),
            ProbeResult.UNAVAILABLE,
            "1970-01-01T00:00:00Z",
        )

    def execute(self, argv, *, attestation):
        return ExecutionResult(None, False, "none", (), "BLOCKED_ISOLATION")
...
class IsolationRegistry:
    ...
    def __init__(self, path):
        self._store = ContractStore(path, IsolationAttestation.from_dict)

    def register(self, attestation):
        if (
            attestation.probe_result is ProbeResult.ENFORCED
            and not attestation.image_digest
        ):
            raise IsolationError("enforced backend requires pinned image")
        return self._store.put(
            f"{attestation.tenant_id}:{attestation.repository_id}:{attestation.backend_id}",
            attestation,
        )

    def get(self, tenant_id, repository_id, backend_id):
        return self._store.get(f"{tenant_id}:{repository_id}:{backend_id}")
