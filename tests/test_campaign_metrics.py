...
def protocol():
    entrants = {
        **{f"MB{i}": recipe(f"MB{i}", "builder-component", "component-default") for i in range(4)},
        **{f"MC{i}": recipe(f"MC{i}", "whole-campaign", "whole-default") for i in range(2)},
    }
    hybrids = {f"MH{i}": recipe(f"MH{i}", "whole-campaign", "whole-default") for i in range(1, 5)}
    manifest = {
        ...
        "block_manifest_digests": {stage: digest([stage, "block"]) for stage in ("original", "hybrid", "final")},
        "task_manifest_digests": {stage: digest([stage, "tasks"]) for stage in ("original", "hybrid", "final")},
        "family_manifest_digests": {stage: digest([stage, "families"]) for stage in ("original", "hybrid", "final")},
        "manifest_signature_refs": {stage: f"signature:{stage}" for stage in ("original", "hybrid", "final")},
        "custody_receipt_ref": "custody:fixture",
    }
    return MatchProtocol(
        "WOS-N02-FIXTURE", "2.0.0", "whole-os-tournament", entrants,
...
def task_bindings(stage):
    ...
    for index in range(count):
        strata = (REQUIRED_STRATA[index],) if index < len(REQUIRED_STRATA) else (REQUIRED_STRATA[0],)
        if count == 12 and index == 0:
            strata = (REQUIRED_STRATA[0], REQUIRED_STRATA[-1])
        rows.append(TaskBinding(f"task-{index:02d}", f"family-{index:02d}", strata, tuple(index * 10 + rep for rep in range(repetitions))))
    return tuple(rows)


PRINCIPALS = AuthorityBinding(
    "evaluator:independent", "custodian:independent", ("builder:a", "builder:b"),
    ("champion:current",), "signature:evaluator", "signature:custody",
)


def stage_evidence(p, stage, *, final_pair=(), stage_opened_at=10, holdout_opened_at=None):
    manifest = p.experiment_manifest
    return StageEvidence(
        p.protocol_digest, manifest["recipe_manifest_digest"], stage,
        {"original":"development-screening", "hybrid":"harder-hybrid-development", "final":"promotion_holdout"}[stage],
        manifest["block_manifest_digests"][stage], manifest["task_manifest_digests"][stage],
        manifest["family_manifest_digests"][stage], manifest["manifest_signature_refs"][stage],
...
def lease(p, stage, *, issued_by="host:lease", issued_at=1, expires_at=1000, revoked_at=None, operations=None):
    operations = frozenset(operations or ("schedule", "apply", "seal", "aggregate", "decide"))
    content = {"protocol": p.protocol_id, "stage": stage, "issuer": issued_by, "issued": issued_at, "expires": expires_at, "revoked": revoked_at, "operations": sorted(operations)}
    return LeaseRecord(digest(content), p.protocol_id, stage, digest([content, "budget"]), issued_by, issued_at, expires_at, revoked_at, operations)


def seal_for(p, evidence, variant, sequence, candidate, sealed_at):
    r = p.recipe(variant)
    return VariantSeal(
        p.protocol_digest, evidence.evidence_digest, sequence, variant,
        p.recipe_digest(variant, r), candidate, PRINCIPALS.evaluator_id,
        PRINCIPALS.custodian_id, evidence.stage, evidence.block_digest,
        evidence.task_manifest_digest, evidence.family_manifest_digest,
        r["regime_id"], sealed_at, f"seal-signature:{sequence}",
    )
...
class _FixtureRegistry:
    ...
    def __init__(self, p, evidence, lease_record, *, observed_at=20, history=()):
        self.handle = _Handle()
        admission = canonical_digest({"protocol_digest": p.protocol_digest, "stage_evidence": evidence, "lease": lease_record})
        self.snapshot = AdmissionSnapshot(admission, p.protocol_digest, evidence, lease_record, frozenset(("host:lease",)), observed_at, tuple(history))
        self.issued = {}
    ...
    def resolve(self, handle, use):
        self._check(handle)
        if use.protocol_digest != self.snapshot.protocol_digest or use.stage != self.snapshot.stage_evidence.stage:
            raise CampaignMetricsError("foreign use")
        if use.state_admission_digest is not None and use.state_admission_digest != self.snapshot.admission_digest:
    ...
    def consume_round(self, handle, state_handle, request):
        ...
        for receipt_handle, receipt in submitted.items():
            ...
            if stored is None or stored.plan != receipt.plan or stored.receipt_digest != receipt.receipt_digest:
    ...
    def record_aggregate(self, handle, evidence, observations):
        self._check(handle)
        if any(row.receipt_digest not in self.measurement_receipts for row in observations):
            raise CampaignMetricsError("aggregate references an unconsumed receipt")
        opaque = _Handle(); receipt = AggregateReceipt(opaque, canonical_digest(evidence))
...
class CampaignMetricsTests(unittest.TestCase):
    def test_production_module_has_no_minter_key_token_or_verifier(self):
        for name in ("admit_protocol", "AdmittedProtocol", "EvidenceVerifier", "SignedCustodyEnvelope", "_ADMISSION_TOKEN"):
    ...
    def test_fixture_closed_document_round_trips_without_becoming_authority(self):
        p = protocol()
        document = {"kind": "hive-mind-closed-match-protocol", "status": "CLOSED_ADMISSION_CANDIDATE", "external_evidence_obligations": [], "protocol_digest": p.protocol_digest, **plain(p.to_document())}
        with tempfile.TemporaryDirectory() as folder:
    ...
    def test_fixture_protocol_is_deeply_frozen_and_all_fields_digest_bound(self):
        p = protocol(); before = p.protocol_digest
        with self.assertRaises(TypeError):
            p.entrant_recipes["MB0"]["track"] = "whole-campaign"
        changed = dict(p.experiment_manifest); changed["selection_seed"] += 1
        p2 = replace(p, experiment_manifest=changed)
        self.assertNotEqual(before, p2.protocol_digest)
        duplicate = dict(p.entrant_recipes); duplicate["MB1"] = dict(duplicate["MB0"])
    ...
    def test_full_admission_identity_and_revalidation(self):
        ...
        mutations = (
            ...
            replace(evidence, principals=replace(PRINCIPALS, evaluator_id="evaluator:other")),
        )
        for changed in mutations:
            new_digest = canonical_digest({"protocol_digest": p.protocol_digest, "stage_evidence": changed, "lease": registry.snapshot.lease})
    ...
    def test_lease_issuer_scope_expiry_revocation_and_operation_rechecked(self):
        p = protocol(); evidence = stage_evidence(p, "original")
        cases = (
    ...
    def test_exact_receipts_full_round_and_durable_replay(self):
        ...
        with self.assertRaises(CampaignMetricsError):
            state.apply(p, (ReceiptOutcome(scheduled.receipts[0], "LEFT"),), registry=registry, admission_handle=registry.handle)
        ...
        for changed_plan in (
            ...
            substituted = (ReceiptOutcome(tampered, "LEFT"),) + tuple(ReceiptOutcome(receipt, "LEFT") for receipt in scheduled.receipts[1:])
            with self.assertRaises(CampaignMetricsError):
                state.apply(p, substituted, registry=registry, admission_handle=registry.handle)
        changed_digest = IssuedReceipt(first.opaque_handle, first.plan, first.issued_at + 1)
        substituted = (ReceiptOutcome(changed_digest, "LEFT"),) + tuple(ReceiptOutcome(receipt, "LEFT") for receipt in scheduled.receipts[1:])
        with self.assertRaises(CampaignMetricsError):
            state.apply(p, substituted, registry=registry, admission_handle=registry.handle)
        ...
        tampered = IssuedReceipt(scheduled.receipts[0].opaque_handle, tampered_plan, scheduled.receipts[0].issued_at)
        with self.assertRaises(CampaignMetricsError):
    ...
    def test_quarantine_both_and_third_loss(self):
        ...
        for receipt in scheduled.receipts:
            if receipt.plan.left == "MB1": outcome = "RIGHT"
            elif receipt.plan.right == "MB1": outcome = "LEFT"
    ...
    def test_final_requires_prior_qualification_same_regime_and_preopen_seals(self):
        ...
        final_evidence = stage_evidence(p, "final", final_pair=finalists, stage_opened_at=10, holdout_opened_at=20)
        final_a = seal_for(p, final_evidence, "MC0", 3, original_seal.candidate_digest, 11)
        final_b = seal_for(p, final_evidence, "MH1", 4, hybrid_seal.candidate_digest, 12)
    ...
    def test_seals_require_order_time_evaluator_stage_block_and_custody(self):
        ...
        validate_and_append_seals(p, (good,), registry=registry, admission_handle=registry.handle)
        ...
        for bad in (
            ...
            with self.assertRaises(CampaignMetricsError):
                validate_and_append_seals(p, (bad,), registry=registry, admission_handle=registry.handle)
        with self.assertRaises(CampaignMetricsError):
    ...
    def test_stage_aggregation_exact_membership_and_registry_bound_decision(self):
        ...
        scheduled = state.schedule(p, registry=registry, admission_handle=registry.handle)
        state.apply(p, tuple(ReceiptOutcome(receipt, "DRAW") for receipt in scheduled.receipts), registry=registry, admission_handle=registry.handle)
        observation_receipts = {task.task_id: digest(["measurement", task.task_id, 0]) for task in evidence.tasks}
        registry.measurement_receipts.update(observation_receipts.values())
        observations = tuple(FamilyObservation(task.family_id, task.task_id, 0, observation_receipts[task.task_id], 0.0) for task in evidence.tasks)
        receipts = []
        ...
        raw_final_like = paired_family_bootstrap({f"family-{index:02d}":(0.0,) for index in range(30)}, seed=1)
        with self.assertRaises(CampaignMetricsError):
    ...
    def test_every_execution_surface_revalidates_lease(self):
        ...
        with self.assertRaises(LeaseExhausted):
            stage_paired_bootstrap(p, "success_difference", "MB0", "MB1", rows, left_hard_gates=True, right_hard_gates=True, registry=registry, admission_handle=registry.handle)
        with self.assertRaises(LeaseExhausted):
            decide_match(p, *aggregates, registry=registry, admission_handle=registry.handle)
