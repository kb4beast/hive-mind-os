...
_CAMPAIGN_TRANSITIONS = {CampaignState.DRAFT: {CampaignState.READY, CampaignState.CANCELLED}, CampaignState.READY: {CampaignState.RUNNING, CampaignState.CANCELLED}, CampaignState.RUNNING: {CampaignState.VERIFYING, CampaignState.FAILED, CampaignState.CANCELLED}, CampaignState.VERIFYING: {CampaignState.COMPLETED, CampaignState.NO_CHANGE, CampaignState.FAILED, CampaignState.CANCELLED}}
_PACKAGE_TRANSITIONS = {PackageState.PROPOSED: {PackageState.READY, PackageState.CANCELLED}, PackageState.READY: {PackageState.RUNNING, PackageState.CANCELLED, PackageState.SUPERSEDED}, PackageState.RUNNING: {PackageState.VERIFYING, PackageState.FAILED, PackageState.CANCELLED}, PackageState.VERIFYING: {PackageState.COMPLETED, PackageState.NO_CHANGE, PackageState.FAILED, PackageState.CANCELLED}}

def require_campaign_transition(current: CampaignState, successor: CampaignState) -> None:
 if not isinstance(current, CampaignState) or not isinstance(successor, CampaignState) or successor not in _CAMPAIGN_TRANSITIONS.get(current, set()): _fail(CampaignContractErrorCode.UNKNOWN_STATE, f"illegal campaign transition {current}->{successor}")

def require_package_transition(current: PackageState, successor: PackageState) -> None:
 if not isinstance(current, PackageState) or not isinstance(successor, PackageState) or successor not in _PACKAGE_TRANSITIONS.get(current, set()): _fail(CampaignContractErrorCode.UNKNOWN_STATE, f"illegal package transition {current}->{successor}")
...
class CandidateCompletion:
 mission_id:str;package_id:str;authority_digest:str;base_commit_sha:str;base_tree_digest:str;commit_sha:str;tree_digest:str;output_receipts:tuple[tuple[str,str],...];acceptance_receipts:tuple[tuple[str,str],...];disposition:str;package_revision:int=0;objective_digest:str=""
 def __post_init__(self):
  ...
  if type(self.package_revision)is not int or self.package_revision<1:_fail(CampaignContractErrorCode.INVALID,"package_revision")
...
class WorkPackage:
 ...
 def __post_init__(self):
  ...
  if self.completion:
   c=self.completion
   if (c.mission_id,c.package_id,c.authority_digest,c.package_revision,c.objective_digest)!=(self.mission_id,self.package_id,self.authority_digest,self.revision,canonical_digest(self.objective)) or set(x[0] for x in c.output_receipts)!=set(self.outputs) or set(x[0] for x in c.acceptance_receipts)!={x.acceptance_id for x in self.acceptance} or (self.state is PackageState.NO_CHANGE)!=(c.disposition=="no-change"):_fail(CampaignContractErrorCode.RECEIPT_INCONSISTENT,"completion binding")
...
def parse_campaign_mission(raw:bytes,*,fixture_mode=False):
    ...
    try:
        ...
        for p in d["packages"]:
            ...
            c=p["completion"]
            if c is not None and set(c) != {"mission_id","package_id","authority_digest","package_revision","objective_digest","base_commit_sha","base_tree_digest","commit_sha","tree_digest","output_receipts","acceptance_receipts","disposition"}: _fail(CampaignContractErrorCode.UNKNOWN_FIELD,"completion")
            completion=None if c is None else CandidateCompletion(c["mission_id"],c["package_id"],c["authority_digest"],c["base_commit_sha"],c["base_tree_digest"],c["commit_sha"],c["tree_digest"],tuple(tuple(x) for x in c["output_receipts"]),tuple(tuple(x) for x in c["acceptance_receipts"]),c["disposition"],c["package_revision"],c["objective_digest"])
