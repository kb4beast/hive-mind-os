...
class CampaignContextError(ValueError): pass
...
class CampaignContextRequest:
    ...
    def __post_init__(self) -> None:
        require_identifier(self.subject_id, "subject_id"); require_identifier(self.node_id, "node_id")
        require_identifier(self.role_purpose, "role_purpose")
        for value in (self.subject_snapshot_digest, self.node_contract_digest, self.objective_digest): require_digest(value, "request digest")
        if not self.permitted_handles or len(set(self.permitted_handles)) != len(self.permitted_handles): raise CampaignContextError("handles must be a nonempty unique tuple")
...
def assemble_campaign_delta(capsule: RoundCapsule, request: CampaignContextRequest) -> tuple[NodeDelta, ContextSelectionReceipt]:
    ...
    delta = capsule.node_delta(request.node_id, node_contract_digest=request.node_contract_digest, objective_digest=request.objective_digest)
    permitted = set(request.permitted_handles)
    visible = {item.context_id for item in delta.direct_bodies} | {item.context_id for item in delta.cold_references}
    if not visible.issubset(permitted): raise CampaignContextError("capsule exposes an unpermitted handle")
    receipt = ContextSelectionReceipt(canonical_digest({"subject":request.subject_id,"node":request.node_id,"contract":request.node_contract_digest,"purpose":request.role_purpose}), tuple(x.context_id for x in delta.direct_bodies), tuple(x.context_id for x in delta.cold_references), delta.omitted_context_ids, "frozen capsule route")
    return delta, receipt

def expand_cold_reference(delta: NodeDelta, reference: ColdContextReference, body: ContextBody, *, reason: str) -> NodeDelta:
