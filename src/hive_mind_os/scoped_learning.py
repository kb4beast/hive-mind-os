...
class LearningRoute:
 ...
 def __post_init__(self):
  ...
  if self.learning_scope is LearningScope.EXPORT_DRAFT and not self.accepted_lesson_digest: raise ValueError("export requires lesson digest")
 @property
...
class DurableChampionRegistry(ChampionRegistry):
 def __init__(self,path):
  super().__init__(); self._store=ContractStore(path,lambda v:v)
 def promote(self,subject,challenger,expected_parent):
  result=super().promote(subject,challenger,expected_parent)
  self._store.put(subject,{"subject":subject,"challenger":challenger.digest,"parent":challenger.parent_digest,"state":challenger.promotion_state.value})
  return result
