...
class WholeOSHost(Protocol):
    ...
    def execute_package(
        self, package: OutcomeWorkPackage, bindings: tuple[Any, Any], payload: Mapping[str, Any]
...
class WholeOSServiceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.binding_descriptor.tenant_id != self.tenant_id or self.binding_descriptor.repository_id != self.repository_id:
...
class WholeOSService:
    ...
    def __init__(
        self, config: WholeOSServiceConfig, bindings: ConfiguredMissionBindingsProvider,
        host: WholeOSHost, *, scheduler: Scheduler | None = None,
    ) -> None:
    ...
    def _jobs(self) -> tuple[Job, ...]:
        return tuple(job for job in self.scheduler.jobs() if job.kind == self.JOB_KIND and job.mission_id == self.config.campaign_id)

    def _completed(self) -> set[str]:
        return {str(job.payload["package_id"]) for job in self._jobs() if job.state == "done"}
        ...
        for package in ready_packages(self.config.graph, completed):
            if package.package_id not in enqueued:
                self.scheduler.enqueue(
                    self.JOB_KIND, self._payload(package), max_attempts=self.config.maximum_attempts,
    ...
    def run_once(self) -> ServiceObservation:
        ...
        if package is None:
            self.scheduler.fail(job.id, job.lease_token or "", "unknown package", mission_id=self.config.campaign_id)
        ...
        try:
            ...
            if result.status in {PackageStatus.COMPLETED, PackageStatus.NO_CHANGE}:
                ...
                self.scheduler.fail(job.id, job.lease_token or "", result.status.value + ": " + result.message, mission_id=self.config.campaign_id)
            return self.observe(last_result=result)
        except Exception as exc:
            self.scheduler.fail(job.id, job.lease_token or "", f"{type(exc).__name__}: {exc}", mission_id=self.config.campaign_id)
            return self.observe(last_result=PackageExecutionResult(package_id, PackageStatus.FAILED, None, (), str(exc)))

    def observe(self, *, last_result: PackageExecutionResult | None = None) -> ServiceObservation:
        jobs = self._jobs()
        completed = tuple(sorted(str(job.payload["package_id"]) for job in jobs if job.state == "done"))
        blocked = tuple(sorted(str(job.payload["package_id"]) for job in jobs if job.state == "dead-letter"))
        ...
        if blocked:
            ...
            status = "idle"
        return ServiceObservation(self.config.campaign_id, status, completed, pending, blocked, last_result)
...
def graph_from_document(document: Mapping[str, object]) -> OutcomeGraphSpec:
    allowed = {"contract_version", "base_snapshot", "maximum_concurrent", "court_receipt", "packages"}
    ...
    for raw in document["packages"]:
        ...
        packages.append(OutcomeWorkPackage(
            ...
            effort=int(raw.get("effort", 1)), rollback=str(raw.get("rollback", "retain prior candidate")),
            state=str(raw.get("state", "pending")),
        ))
    return compile_outcome_graph(OutcomeGraphSpec(
        packages=tuple(packages), base_snapshot=str(document["base_snapshot"]),
        contract_version=int(document["contract_version"]),
        maximum_concurrent=int(document["maximum_concurrent"]), court_receipt=str(document["court_receipt"]),
    ))
