...
non-merge direct child of `28463ae`, changing exactly seven predecessor files:
`.gitattributes`, `.github/workflows/ci.yml`, `ADR_INDEX.md`, this README,
`manifest.json`, `verify_plan.py`, and `tests/test_generic_dag_v3_overlay.py`.
It also adds exactly ADR-070, ADR-071, the thin Git bundle, and its provenance
record. Each changed blob must differ from the parent. ADR-069,
...
The manifest embeds sixteen non-manifest bindings. An external caller supplies the raw
manifest digest, completing the non-circular 17-path inventory, and independently
...
For authoring, use that raw-byte checkout at exact parent `28463ae`, overlay the 17
current payload members, and leave exactly the eleven V5 paths uncommitted. Seven are
modified predecessor files; four are new, self-contained history/decision files:
`ADR-070`, `ADR-071`, the thin Git bundle, and its provenance record.
...
After the exact eleven-path child is committed directly on `28463ae`, use the same
...
authoring state, wrong parent or extra commit, an imprecise eleven-path diff, changed
