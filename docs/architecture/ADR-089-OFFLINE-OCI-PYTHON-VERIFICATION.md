...
### 2026-09-21 full-suite test-isolation correction

The first integrated local run on source commit 8e5dd30acf84e99f592e7b336528a2d987ae92c8 executed 2,179 tests in 2,559.033 seconds and failed one test, with 13 skips. The new pipe-cleanup test called global `gc.collect()` inside a ResourceWarning capture and attributed 21 unreachable SQLite connections from earlier tests to its own child pipes. The failure and full log remain in `oci-second-remand-full-ci.log`; this run is not a pass.

Independent reproduction seeded 21 unrelated SQLite connections and reproduced the false failure. More seriously, keeping the child Popen objects alive let the original warning-based test miss four actual leaked handles. The repair retains only this test's actual child processes and asserts that each is reaped and both real output handles are closed before test cleanup. It removes the global garbage-collection warning oracle; it does not suppress warnings or change production execution. An independent fault injection disabling pipe closure must still fail the repaired assertion for normal exit and timeout. Focused discovery after the repair: 78 tests, OK with the existing one Windows privilege skip. The full integrated gate and independent installed-test review are rerun separately; no passing result is presumed here.

Installed-test independent Curator disposition: ADOPT, limited to the test correction. Report `oci-pipe-installed-curator-review.md`, SHA256 `2e082eb9ab5c65f689a91497772d604f7dc1a656ac202391c921f7267871623d`, binds final test SHA256 `2e7538915cced08c1f14324f20e5adcba92bcbbe1a4286d231d6e20cbc92c35e`. Clean and forced-21-SQLite-warning runs pass; separately omitting stdout closure, stderr closure, both closures, or child reaping each causes both expected normal/timeout subtest failures. Cleanup succeeds after those failures. All eight runner tests pass, Ruff/Pyright pass, and both production module hashes remain unchanged. A premature full rerun was deliberately stopped for a typing-only wrapper annotation correction; the current unchanged executable test body is now being exercised in `oci-pipe-isolation-full-ci-v2.log`. Neither the interrupted nor in-progress run counts as a completed gate.
