...
ROLES=("orchestrator","explorer","architect","builder","curator","integrator","steward","optimizer")
@dataclass(frozen=True, slots=True)
class RoleRow:
    role:str; identity:str; packages:tuple[str,...]=(); evidence:tuple[str,...]=(); mode:str="direct"; revisit_triggers:tuple[str,...]=()
@dataclass(frozen=True, slots=True)
...
class RoleCoverageReceipt:
    plan_digest:str; results:tuple[tuple[str,str,str],...]; complete:bool; missing:tuple[str,...]=()
    @property
    def digest(self): return canonical_digest(self)
def build_coverage(plan:RoleCoveragePlan, results:dict[str,tuple[str,str]])->RoleCoverageReceipt:
    rows=[]; missing=[]
    for role in ROLES:
