...
class EndpointSeal:
 ...
 def from_dict(cls,value):
  fields={"episode_id","input_manifest_digest","candidate_tree_digest","learner_variant_digest","prompt_memory_digest","tool_profile_digest","rubric_digest","budget_digest","learner_id","evaluator_id","sealed_at","seal_digest"}
  if set(value)!=fields: raise CustodyError("closed endpoint seal schema")
  return cls(**value)
@dataclass(frozen=True, slots=True)
...
class EndpointSealStore:
 def __init__(self,path): self._store=ContractStore(path,EndpointSeal.from_dict)
 def seal_once(self,seal):
  key=seal.episode_id
  old=self._store.get(key)
  if old and old.seal()!=seal.seal(): raise CustodyError("episode already sealed with different inputs")
  return self._store.put(key,seal)
 def get(self,episode_id): return self._store.get(episode_id)
