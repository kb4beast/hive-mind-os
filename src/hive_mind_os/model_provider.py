...
class AnthropicProvider(_BaseProvider):
    ...
    def _parse(self, raw: bytes, api_key: str, retry_index: int) -> ModelResponse:
        ...
        try:
            ...
            raise ModelResponseError(
                "Anthropic response lacks text content", raw
            ) from None
        if not isinstance(content, str):
...
def provider_from_env(
    ...
    *,
    ...
    config = ProviderConfig(
        ...
        api_key_env=(_role_env("HIVE_MIND_MODEL_API_KEY_ENV", role) or default_key_env),
        timeout_s=_env_float("HIVE_MIND_MODEL_TIMEOUT_S", 60.0, role),
        max_output_tokens=_env_int("HIVE_MIND_MODEL_MAX_OUTPUT_TOKENS", 2048, role),
