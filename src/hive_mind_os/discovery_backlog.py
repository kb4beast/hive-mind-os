...
class BacklogCandidate:
    ...
    def __post_init__(self):
        if not self.idea_id or not self.atomic_claims or not self.success_measure:
            raise ValueError("candidate evidence and acceptance are required")
        if self.effort <= 0 or self.user_impact < 0 or not 0 <= self.risk <= 1:
            raise ValueError("invalid candidate score inputs")
...
class DiscoveryBacklog:
    def __init__(self, state_dir: str | Path | None = None):
        ...
        if state_dir is not None:
            ...
            self._db = sqlite3.connect(p / "discovery.sqlite3")
            self._db.execute(
    ...
    def ingest(self, signal: DiscoverySignal) -> bool:
        ...
        if key in self.signals or (
            ...
            and self._db.execute(
                "SELECT 1 FROM signals WHERE k=?", (json.dumps(key),)
            ).fetchone()
        ):
            return False
        ...
        if self._db:
            ...
            self._db.execute(
                ...
                (json.dumps(key), json.dumps(body, sort_keys=True, default=str)),
            )
            self._db.commit()
        return True
    ...
    def select(self, *, limit: int = 1) -> BacklogSelection:
        ...
        chosen = tuple(i for i, _ in scored[:limit])
        return BacklogSelection(
            tuple(self.candidates.values()),
            chosen,
            scored,
            tuple((i, "lower evidence-adjusted value") for i, _ in scored[limit:]),
        )
