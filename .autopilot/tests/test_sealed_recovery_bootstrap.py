...
class SealedRecoveryBootstrapTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        if self.original_capability == "0" * 40:
            ...
            builder_path = (
                self.root / ".autopilot" / "builder-330-recovery-bootstrap.json"
            )
    ...
    def _record(self, node_id: str) -> dict:
        ...
        return next(
            record
            for record in document["repair_authorities"]
    ...
    def _live_snapshot(self, node_id: str) -> dict:
        ...
        return {
            ...
            "branches": [
                {"name": record["branch"], "sha": record["old_receipt_commit"]}
            ],
        }
    ...
    def test_exact_repair_records_and_builder_court_chain_validate(self) -> None:
        ...
        for record in records.values():
            ...
            self.assertTrue(
                record["authenticated_snapshot_retrieval_digest"].startswith("sha256:")
            )
    ...
    def test_builder_exact_records_reject_tamper_and_explorer_substitution(
        self,
    ) -> None:
        ...
        for node_id in ("OPTIMIZER-370", "ORCH-300"):
            ...
            for key, value in (
                ...
                with self.subTest(node_id=node_id, key=key):
                    self.assertTrue(
                        SealedRecoveryMixin._repair_live_issues(self.plane, record)
                    )
    ...
    def test_repair_claim_requires_remote_publication_and_literal_origin(self) -> None:
        for node_id in ("OPTIMIZER-370", "ORCH-300"):
            ...
            with self.assertRaises(autopilot.ClaimError):
                self.plane.claim_internal(node_id, "test:owner", publish_remote=False)
            with self.assertRaises(autopilot.ClaimError):
                self.plane.claim_internal(
                    node_id, "test:owner", publish_remote=True, remote="Origin"
                )
    ...
    def test_exact_claim_cas_is_single_use_and_rollback_restores_old_head(self) -> None:
        ...
        self.plane._create_repair_claim_commits = lambda _record, _local: (
            "c" * 40,
            "d" * 40,
        ...
        claim = self.plane.claim_internal(
            "OPTIMIZER-370", "test:owner", publish_remote=True
        )
        self.assertEqual(claim["remote_claim_commit"], "c" * 40)
        self.assertEqual(claim["remote_head_commit"], "d" * 40)
        with self.assertRaises(autopilot.ClaimError):
            self.plane.claim_internal(
                "OPTIMIZER-370", "test:other", publish_remote=True
            )
        self.plane.release_internal(
            "OPTIMIZER-370",
            "test:owner",
            claim_id=str(claim["claim_id"]),
            reason="bounded rollback",
        )
    ...
    def test_failed_claim_compensation_preserves_adverse_lease_evidence(self) -> None:
        ...
        try:
            ...
            sealed_recovery.atomic_write_json = original_write
        retained = json.loads(
            self.plane.claim_path("OPTIMIZER-370").read_text(encoding="utf-8")
        )
    ...
    def test_live_origin_release_race_after_claim_cas_restores_old_head(self) -> None:
        ...
        with self.assertRaisesRegex(
            autopilot.ClaimError, "literal origin release moved"
        ):
            self.plane.claim_internal(
                "OPTIMIZER-370", "test:owner", publish_remote=True
            )
    ...
    def test_preparing_prepared_and_expired_claim_restart_states_recover_exact_head(
        self,
    ) -> None:
        record = self._record("OPTIMIZER-370")
        ...
        autopilot.atomic_write_json(
            path, {**base, "status": "PREPARING", "remote_claim_commit": None}
        )
    ...
    def test_optimizer_and_orch_claim_topologies_are_distinct_and_exact(self) -> None:
        ...
        self.plane._git = lambda args, **_kwargs: subprocess.CompletedProcess(
            args, 0, "f" * 40 + "\n", ""
        )  # type: ignore[method-assign]
        ...
        self.assertEqual(
            set(
                self.plane._diff_paths("cfe17ff7d6b06bdaa42e9ba6ec2a75a9c66c6a58", tree)
            ),
            set(orch["allowed_paths"]),
        )

    def test_clean_single_branch_clone_fetches_pinned_old_head_before_tree_work(
        self,
    ) -> None:
        seed = self.root / "seed"
        ...
        copy_autopilot_fixture(
            Path(__file__).resolve().parents[1], clone / ".autopilot"
        )
    ...
    def _consultation() -> dict:
        return {
            "request_id": "CONSULT-sealed",
            "mission_id": "MISSION-sealed",
            "question": "Is this recovery evidence sufficient?",
            "reason_code": "AMBIGUITY",
            "requesting_role": "builder",
            "consulted_roles": ["architect", "curator"],
            "round": 1,
            "suspected_cheating": False,
            "evidence_refs": ["evidence:sealed"],
            "decision": "RESOLVED",
            "answer": "Resolved from exact evidence",
            "dissent": [],
    ...
    def test_replacement_requires_supersedes_grant_exact_pr_and_scope(self) -> None:
        for node_id in ("OPTIMIZER-370", "ORCH-300"):
            self.assertEqual(
                self.plane._replacement_receipt_issues(node_id, self._receipt(node_id)),
                (),
            )
            for mutation in ("missing_supersedes", "wrong_grant", "wrong_pr", "scope"):
                ...
                with self.subTest(node_id=node_id, mutation=mutation):
                    self.assertTrue(
                        self.plane._replacement_receipt_issues(node_id, receipt)
                    )
    ...
    def test_replacement_receipt_rejects_schema_expansion_and_weak_types(self) -> None:
        def consultation_mutation(receipt: dict, mutation: str) -> None:
            ...
            if mutation == "extra_consultation_key":
                ...
                consultation["identity_records"].append(
                    copy.deepcopy(consultation["identity_records"][0])
                )
                ...
                consultation["identity_records"][1]["identity"] = consultation[
                    "identity_records"
                ][0]["identity"]
        ...
        self.assertEqual(
            self.plane._replacement_receipt_issues("OPTIMIZER-370", valid), ()
        )

        mutations = {
            "extra_top_level": lambda receipt: receipt.__setitem__("unexpected", True),
            "blank_identity": lambda receipt: receipt["role_identities"][0].__setitem__(
                "identity", "  "
            ),
            "missing_runtime": lambda receipt: receipt.__setitem__(
                "model_runtime", None
            ),
            "non_string_evidence": lambda receipt: receipt.__setitem__(
                "evidence_refs", [1]
            ),
            "extra_test_key": lambda receipt: receipt["tests"][0].__setitem__(
                "unexpected", True
            ),
            "extra_runtime_key": lambda receipt: receipt["model_runtime"].__setitem__(
                "unexpected", True
            ),
            ...
            "extra_consultation_key": lambda receipt: consultation_mutation(
                receipt, "extra_consultation_key"
            ),
            "extra_identity_key": lambda receipt: consultation_mutation(
                receipt, "extra_identity_key"
            ),
            "duplicate_identity": lambda receipt: consultation_mutation(
                receipt, "duplicate_identity"
            ),
            "duplicate_consulted_role": lambda receipt: consultation_mutation(
                receipt, "duplicate_consulted_role"
            ),
            "blank_consultation_identity": lambda receipt: consultation_mutation(
                receipt, "blank_consultation_identity"
            ),
            "typed_consultation_identity": lambda receipt: consultation_mutation(
                receipt, "typed_consultation_identity"
            ),
            "reused_consultation_identity": lambda receipt: consultation_mutation(
                receipt, "reused_consultation_identity"
            ),
            "wrong_consultation_identity_kind": lambda receipt: consultation_mutation(
                receipt, "wrong_consultation_identity_kind"
            ),
            "requester_self_consultation": lambda receipt: consultation_mutation(
                receipt, "requester_self_consultation"
            ),
        }
        for name, mutate in mutations.items():
            ...
            with self.subTest(name=name):
                self.assertTrue(
                    self.plane._replacement_receipt_issues("OPTIMIZER-370", receipt)
                )
    ...
    def test_canonical_receipt_schema_mutation_matrix_fails_closed(self) -> None:
        ...
        self.assertEqual(
            self.plane._sealed_receipt_shape_issues(node_id, baseline()), ()
        )

        mutations: list[tuple[str, Callable[[dict], object]]] = []
        for key in baseline():
            mutations.append(
                (f"missing_top_level_{key}", lambda value, key=key: value.pop(key))
            )
        for key in (
            "plan_fingerprint",
            "node_id",
            ...
            mutations.append(
                (
                    f"blank_top_level_{key}",
                    lambda value, key=key: value.__setitem__(key, " "),
                )
            )
        for key in ("provider", "model"):
            mutations.append(
                (
                    f"blank_runtime_{key}",
                    lambda value, key=key: value["model_runtime"].__setitem__(key, " "),
                )
            )
        for key in ("role", "identity", "identity_kind"):
            mutations.append(
                (
                    f"blank_role_{key}",
                    lambda value, key=key: value["role_identities"][0].__setitem__(
                        key, " "
                    ),
                )
            )
        for key in ("node_id", "autonomy_level", "grant_id"):
            mutations.append(
                (
                    f"blank_authority_{key}",
                    lambda value, key=key: value["authority"].__setitem__(key, " "),
                )
            )
        for key in (
            ...
            mutations.append(
                (
                    f"blank_authority_binding_{key}",
                    lambda value, key=key: value["authority"].__setitem__(key, " "),
                )
            )
        for key in (
            ...
            mutations.append(
                (
                    f"blank_consultation_{key}",
                    lambda value, key=key: value["consultations"][0].__setitem__(
                        key, " "
                    ),
                )
            )
        for key in ("role", "identity", "identity_kind"):
            mutations.append(
                (
                    f"blank_consultation_identity_{key}",
                    lambda value, key=key: value["consultations"][0][
                        "identity_records"
                    ][0].__setitem__(key, " "),
                )
            )
        mutations.extend(
            [
                ...
                (
                    "commit_short",
                    lambda value: value.__setitem__("final_commit", "abc"),
                ),
                ...
                (
                    "timestamp_naive",
                    lambda value: value.__setitem__("timestamp", "2030-01-01T00:00:00"),
                ),
                (
                    "timestamp_offset",
                    lambda value: value.__setitem__(
                        "timestamp", "2029-12-31T18:00:00-06:00"
                    ),
                ),
                (
                    "timestamp_space",
                    lambda value: value.__setitem__(
                        "timestamp", "2030-01-01 00:00:00Z"
                    ),
                ),
                (
                    "timestamp_fraction",
                ...
                (
                    "rollback_blank",
                    lambda value: value.__setitem__("rollback_ref", " "),
                ),
                (
                    "decision_unknown",
                    lambda value: value.__setitem__("acceptance_decision", "ADOPT"),
                ),
                ("paths_typed", lambda value: value.__setitem__("changed_paths", None)),
                (
                    "paths_blank",
                    lambda value: value.__setitem__("changed_paths", [" "]),
                ),
                (
                    "paths_duplicate",
                    lambda value: value["changed_paths"].append(
                ...
                (
                    "evidence_duplicate",
                    lambda value: value.__setitem__("evidence_refs", ["e", "e"]),
                ),
                (
                    "runtime_extra",
                    lambda value: value["model_runtime"].__setitem__("extra", True),
                ),
                ("runtime_missing", lambda value: value["model_runtime"].pop("model")),
                (
                    "runtime_blank",
                    lambda value: value["model_runtime"].__setitem__("provider", " "),
                ),
                (
                    "test_extra",
                    lambda value: value["tests"][0].__setitem__("extra", True),
                ),
                ("test_missing", lambda value: value["tests"][0].pop("command")),
                (
                    "test_blank",
                    lambda value: value["tests"][0].__setitem__("name", " "),
                ),
                (
                    "test_status",
                    lambda value: value["tests"][0].__setitem__("status", "failed"),
                ),
                (
                    "test_status_blank",
                    lambda value: value["tests"][0].__setitem__("status", " "),
                ),
                (
                    "test_command_type",
                    lambda value: value["tests"][0].__setitem__("command", [1]),
                ),
                (
                    "test_command_blank",
                    lambda value: value["tests"][0].__setitem__("command", [" "]),
                ),
                ("test_order", lambda value: value["tests"].reverse()),
                (
                    "role_extra",
                    lambda value: value["role_identities"][0].__setitem__(
                        "extra", True
                    ),
                ),
                (
                    "role_missing",
                    lambda value: value["role_identities"][0].pop("identity"),
                ),
                (
                    "role_blank",
                    lambda value: value["role_identities"][0].__setitem__(
                ...
                (
                    "role_reused_identity",
                    lambda value: value["role_identities"][1].__setitem__(
                ...
                (
                    "authority_extra",
                    lambda value: value["authority"].__setitem__("extra", True),
                ),
                ("authority_missing", lambda value: value["authority"].pop("grant_id")),
                (
                    "authority_grants_type",
                    lambda value: value["authority"].__setitem__("grants", "grant"),
                ),
                (
                    "authority_grants_duplicate",
                    lambda value: value["authority"].__setitem__("grants", ["g", "g"]),
                ),
                (
                    "authority_grants_blank",
                    lambda value: value["authority"].__setitem__("grants", [" "]),
                ),
                (
                    "authority_blank",
                    lambda value: value["authority"].__setitem__("grant_id", " "),
                ),
                (
                    "authority_sha",
                    lambda value: value["authority"].__setitem__(
                        "repair_claim_commit", "A" * 40
                    ),
                ),
                (
                    "authority_digest",
                ...
                (
                    "consultation_extra",
                    lambda value: value["consultations"][0].__setitem__("extra", True),
                ),
                (
                    "consultation_missing",
                    lambda value: value["consultations"][0].pop("reason_code"),
                ),
                (
                    "consultation_blank",
                    lambda value: value["consultations"][0].__setitem__(
                        "question", " "
                    ),
                ),
                (
                    "consultation_round_bool",
                    lambda value: value["consultations"][0].__setitem__("round", True),
                ),
                (
                    "consultation_bool_type",
                    lambda value: value["consultations"][0].__setitem__(
                        "suspected_cheating", 0
                    ),
                ),
                (
                    "consultation_array_type",
                    lambda value: value["consultations"][0].__setitem__(
                        "evidence_refs", "evidence"
                    ),
                ),
                (
                    "consultation_array_duplicate",
                    lambda value: value["consultations"][0].__setitem__(
                        "dissent", ["same", "same"]
                    ),
                ),
                (
                    "consultation_evidence_blank",
                    lambda value: value["consultations"][0].__setitem__(
                        "evidence_refs", [" "]
                    ),
                ),
                (
                    "consultation_dissent_blank",
                    lambda value: value["consultations"][0].__setitem__(
                        "dissent", [" "]
                    ),
                ),
                ...
                (
                    "consultation_answer_type",
                    lambda value: value["consultations"][0].__setitem__("answer", 1),
                ),
                (
                    "consultation_answer_blank",
                    lambda value: value["consultations"][0].__setitem__("answer", " "),
                ),
                (
                    "consultation_authority_blank",
                    lambda value: value["consultations"][0].__setitem__(
                        "authority_class", " "
                    ),
                ),
                (
                    "consultation_role_blank",
                    lambda value: value["consultations"][0][
                        "consulted_roles"
                    ].__setitem__(0, " "),
                ),
                (
                    ...
                    lambda value: value["consultations"][0][
                        "consulted_roles"
                    ].__setitem__(0, "builder"),
                ),
                (
                    "consultation_duplicate_role",
                    lambda value: value["consultations"][0].__setitem__(
                ...
                (
                    "identity_extra",
                    lambda value: value["consultations"][0]["identity_records"][
                        0
                    ].__setitem__("extra", True),
                ),
                (
                    "identity_missing",
                    lambda value: value["consultations"][0]["identity_records"][0].pop(
                        "identity_kind"
                    ),
                ),
                (
                    ...
                    lambda value: value["consultations"][0]["identity_records"][
                        0
                    ].__setitem__("identity", 1),
                ),
                (
                    "identity_kind",
                    lambda value: value["consultations"][0]["identity_records"][
                        0
                    ].__setitem__("identity_kind", "other"),
                ),
                (
                    "identity_reuse",
                    lambda value: value["consultations"][0]["identity_records"][
                        ...
                        "identity",
                        value["consultations"][0]["identity_records"][0]["identity"],
                    ),
                ),
                (
                    "identity_order",
                ...
                (
                    "duplicate_request",
                    lambda value: value["consultations"].append(
        ...
        for label, mutate in mutations:
            ...
            with self.subTest(label=label):
                self.assertTrue(
                    self.plane._sealed_receipt_shape_issues(node_id, receipt)
                )

    def test_only_exact_old_new_pair_resolves_and_extra_receipt_fails_closed(
        self,
    ) -> None:
        node_id = "ORCH-300"
        ...
        records = [old, new]
        self.assertEqual(
            self.plane.resolve_sealed_repair_records(node_id, records), [new]
        )
        ...
        self.assertIs(
            self.plane.resolve_sealed_repair_records(node_id, wrong_pair), wrong_pair
        )

    def test_integrated_exact_old_new_durable_pair_projects_complete_without_validator_stub(
        self,
    ) -> None:
        node_id = "OPTIMIZER-370"
        record = copy.deepcopy(self._record(node_id))
        ...
        replacement["authority"]["repair_authority_digest"] = autopilot.digest_json(
            record
        )
        durable_records = {
            node_id: [
                {
                    "commit": record["old_receipt_commit"],
                    "parents": (record["candidate_commit"],),
                    "tree": record["old_receipt_tree"],
                    "receipt": old_receipt,
                },
                {
                    "commit": "f" * 40,
                    "parents": (replacement["final_commit"],),
                    "tree": replacement["final_tree"],
                    "receipt": replacement,
                },
            ]
        }
    ...
    def test_real_bare_optimizer_claim_restart_receipt_and_integrated_complete(
        self,
    ) -> None:
        ...
        control = json.loads(
            (repository / ".autopilot" / "control-plane.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        original_claim = run(
            ...
            target,
            "-m",
            json.dumps(original_claim_payload, sort_keys=True, separators=(",", ":")),
        )
        ...
        old_receipt = run(
            ...
            + json.dumps(old_receipt_payload, sort_keys=True, separators=(",", ":")),
        )
        ...
        record.update(
            {
                ...
                "old_receipt_payload_digest": autopilot.digest_json(
                    old_receipt_payload
                ),
            }
        )
        ...
        def configure(plane) -> None:
            plane._repair_records = lambda: {  # type: ignore[method-assign]
                "OPTIMIZER-370": record,
                "ORCH-300": other_record,
            }
            ...
            plane.current_release = lambda: {  # type: ignore[method-assign]
                "release_id": "sealed-real-release",
                "target_sha": target,
            }
        ...
        ready_runtime(controller_module, repository)
        configure(plane)
        claim = plane.claim_internal(
            "OPTIMIZER-370", "test:real-flow", publish_remote=True
        )
        ...
        integrated.current_release = lambda: {  # type: ignore[method-assign]
            "release_id": "integrated",
            "target_sha": published_receipt,
        }
        view = integrated.node_view("OPTIMIZER-370")
        self.assertEqual(view.state, "COMPLETE", view.reasons)

    def test_orch_exact_inherited_claim_provenance_can_validate_integrated_successor(
        self,
    ) -> None:
        node_id = "ORCH-300"
        record = self._record(node_id)
        ...
        self.plane.is_ancestor = lambda ancestor, descendant: (  # type: ignore[method-assign]
            (ancestor, descendant)
            in {(final, "f" * 40), ("f" * 40, "9" * 40), (original, final)}
        )
        ...
        self.assertEqual(
            self.plane._validate_receipt_commit_record(node_id, durable), ()
        )

    def test_receipt_publication_uses_final_head_cas_and_keeps_claim_until_verified(
        self,
    ) -> None:
        node_id = "OPTIMIZER-370"
        record = self._record(node_id)
        ...
        self.plane._release_binding_issues = lambda _claim=None: ()  # type: ignore[method-assign]
        self.plane._replacement_receipt_issues = lambda *_args, **_kwargs: ()  # type: ignore[method-assign]
        self.plane.acquire_global_validation_lease_internal = lambda *_args, **_kwargs: {
            "lease_id": "lease"
        }  # type: ignore[method-assign]
        self.plane.release_global_validation_lease_internal = lambda *_args, **_kwargs: (
            None
        )  # type: ignore[method-assign]
        ...
        self.plane._cas_update_branch = cas  # type: ignore[method-assign]
        committed = self.plane.complete_internal(
            node_id, "test:owner", receipt, claim_id=CLAIM_ID
        )
    ...
    def test_receipt_cas_failure_restores_claimed_state_without_completion_evidence(
        self,
    ) -> None:
        node_id = "OPTIMIZER-370"
        record = self._record(node_id)
    ...
    def test_restart_after_local_receipt_creation_rolls_back_and_retries(self) -> None:
        ...
        def git(args, **_kwargs):
            ...
            if command[:3] == ("show", "-s", "--format=%B"):
                return subprocess.CompletedProcess(
                    command, 0, self.plane._receipt_message(receipt), ""
                )
            return subprocess.CompletedProcess(command, 0, "", "")
    ...
    def test_hard_restart_after_receipt_remote_cas_resumes_local_evidence_cleanup(
        self,
    ) -> None:
        node_id = "OPTIMIZER-370"
        record = self._record(node_id)
        ...
        self.plane._commit_tree = lambda _sha: receipt["final_tree"]  # type: ignore[method-assign]
        self.plane._git = (
            lambda args, **_kwargs: subprocess.CompletedProcess(  # type: ignore[method-assign]
                args,
                0,
        ...
        self.plane._live_release_issues = lambda _record, _expected=None: (
            "literal origin release advanced",
        )  # type: ignore[method-assign]
        self.plane.clock = lambda: autopilot.parse_time("2031-01-01T00:00:00Z")  # type: ignore[method-assign]
        recovered = self.plane.complete_internal(
            node_id, "test:owner", receipt, claim_id=CLAIM_ID
        )
    ...
    def test_receipt_post_cas_rollback_failure_preserves_adverse_claim_and_intent(
        self,
    ) -> None:
        node_id = "OPTIMIZER-370"
        record = self._record(node_id)
        ...
        self.assertTrue(
            (
                self.plane.state_dir
                / f"sealed-repair-completion-{node_id.lower()}.json"
            ).is_file()
        )
    ...
    def test_builder_atomic_cas_archive_delete_and_reuse_denial(self) -> None:
        ...
        self.plane._remote_ref_sha = lambda ref: (
            state["source"] if ref.startswith("refs/heads/") else state["archive"]
        )  # type: ignore[method-assign]
        ...
        def git(args, *, check=False, environment=None):
            ...
            if command[0] == "rev-parse":
                return subprocess.CompletedProcess(
                    command, 0, replan["candidate_commit"] + "\n", ""
                )
            if command[0] == "update-ref":
                return subprocess.CompletedProcess(command, 0, "", "")
    ...
    def test_builder_post_push_failure_atomically_restores_source_and_removes_archive(
        self,
    ) -> None:
        replan = self.plane._builder_document(BUILDER_REPLAN_PATH)
        assert isinstance(replan, dict)
        ...
        self.plane._git = git  # type: ignore[method-assign]
        self.plane._commit_tree = lambda _sha: (_ for _ in ()).throw(
            RuntimeError("post-push verification failed")
        )  # type: ignore[method-assign]
        ...
        self.assertIn(
            f"--force-with-lease=refs/heads/{replan['branch']}:", compensation
        )
        self.assertIn(
            f"--force-with-lease={replan['archive_ref']}:{replan['candidate_commit']}",
            compensation,
        )
    ...
    def test_builder_prepared_intent_recovers_verified_post_push_state(self) -> None:
        ...
        lease = {
            ...
            "node_id": "BUILDER-330",
            "actor": "test:builder",
            "source_head": replan["candidate_commit"],
        ...
        intent = {
            ...
            "actor": "test:builder",
            "lease_digest": autopilot.digest_json(lease),
    ...
    def test_builder_restart_after_execution_record_finishes_only_intent_cleanup(
        self,
    ) -> None:
        replan = self.plane._builder_document(BUILDER_REPLAN_PATH)
        assert isinstance(replan, dict)
    ...
    def test_builder_recovery_snapshot_binds_absent_source_and_exact_archive(
        self,
    ) -> None:
        replan = self.plane._builder_document(BUILDER_REPLAN_PATH)
        assert isinstance(replan, dict)
        snapshot_digest = autopilot.digest_json({"snapshot": "fresh"})
        reconciliation_digest = autopilot.digest_json({"reconciliation": "fresh"})
        doctor_digest = autopilot.digest_json({"doctor": "fresh"})
        self.plane.current_target_sha = lambda: "d" * 40  # type: ignore[method-assign]
        self.plane._snapshot_digest = lambda: snapshot_digest  # type: ignore[method-assign]
        self.plane._reconciliation_digest = lambda: reconciliation_digest  # type: ignore[method-assign]
        self.plane._doctor_evidence_digest = lambda: doctor_digest  # type: ignore[method-assign]
        execution = {
            "schema_version": 1,
            "kind": sealed_recovery.BUILDER_EXECUTION_KIND,
            "status": "RETIRED",
            ...
            "reconciliation_digest": "old-reconciliation",
            "doctor_evidence_digest": "old-doctor",
            "target_sha": "d" * 40,
            "release_id": "release",
            "actor": "test:builder",
        ...
        autopilot.atomic_write_json(self.plane.builder_execution_path, execution)
        self.plane._write_builder_recovery(recovery)
        ...
        self.plane.github_snapshot = lambda: {  # type: ignore[method-assign]
            "branches": [],
            "refs": [
                {"name": replan["archive_ref"], "sha": replan["candidate_commit"]}
            ],
        }
        self.assertEqual(self.plane._builder_recovery_issues(), ())

    def test_builder_recovery_uses_execution_authority_and_ignores_local_decoy(
        self,
    ) -> None:
        plane = autopilot.ControlPlane(self.root)
        self.assertNotEqual(
            plane.builder_recovery_path,
            self.root / ".autopilot" / "state" / sealed_recovery.BUILDER_RECOVERY_FILE,
        )
        replan = plane._builder_document(BUILDER_REPLAN_PATH)
        assert isinstance(replan, dict)
        snapshot_digest = autopilot.digest_json({"snapshot": "canonical"})
        reconciliation_digest = autopilot.digest_json({"reconciliation": "canonical"})
        doctor_digest = autopilot.digest_json({"doctor": "canonical"})
        plane.current_target_sha = lambda: "d" * 40  # type: ignore[method-assign]
        plane._snapshot_digest = lambda: snapshot_digest  # type: ignore[method-assign]
        plane._reconciliation_digest = lambda: reconciliation_digest  # type: ignore[method-assign]
        plane._doctor_evidence_digest = lambda: doctor_digest  # type: ignore[method-assign]
        execution = {
            "schema_version": 1,
            "kind": sealed_recovery.BUILDER_EXECUTION_KIND,
            "status": "RETIRED",
            ...
            "source_head": replan["candidate_commit"],
            "archive_ref": replan["archive_ref"],
            "snapshot_digest": autopilot.digest_json({"snapshot": "old"}),
            "reconciliation_digest": autopilot.digest_json({"reconciliation": "old"}),
            "doctor_evidence_digest": autopilot.digest_json({"doctor": "old"}),
            "target_sha": "d" * 40,
            "release_id": "release",
            "actor": "test:builder",
            "completed_at": "2026-08-11T00:00:00Z",
        }
        autopilot.atomic_write_json(plane.builder_execution_path, execution)
        plane._write_builder_recovery(
            {
                "schema_version": 1,
                "kind": sealed_recovery.BUILDER_EXECUTION_KIND,
                "recovery_id": execution["recovery_id"],
        ...
        autopilot.atomic_write_json(decoy, {"stale": True})
        plane.github_snapshot = lambda: {  # type: ignore[method-assign]
            "branches": [],
            "refs": [
                {
                    "name": replan["archive_ref"],
                    "sha": replan["candidate_commit"],
                }
            ],
        }

        self.assertEqual(plane._builder_recovery_issues(), ())
        self.assertEqual(autopilot.read_json(decoy), {"stale": True})
    ...
    def test_builder_source_archive_pr_and_lease_mismatches_never_push(self) -> None:
        ...
        cases = (
            ...
            (
                {
                    "pull_requests": [{"node_id": "BUILDER-330"}],
                    "branches": [
                        {
                            "name": replan["branch"],
                            "sha": replan["candidate_commit"],
        ...
        for snapshot, _label in cases:
            self.plane.github_snapshot = lambda snapshot=snapshot: {
                "target_sha": "d" * 40,
                **snapshot,
            }  # type: ignore[method-assign]
    ...
    def test_builder_active_claim_validation_lease_archive_and_foreign_lease_fail_closed(
        self,
    ) -> None:
        replan = self.plane._builder_document(BUILDER_REPLAN_PATH)
        assert isinstance(replan, dict)
        ...
        claim_path.unlink()
        autopilot.atomic_write_json(
            self.plane.validation_lease_path, {"owner": "foreign"}
        )
