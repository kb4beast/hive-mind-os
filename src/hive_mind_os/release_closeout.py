...
from .whole_os_qualification import (
    CloseoutManifest, Disposition, EvidenceKind, EvidenceRef, ExternalObligation,
...
class CloseoutBuilder:
    ...
    def seal(
        ...
        if not startup_command or not rollback_command or any(type(part) is not str or not part or "\x00" in part for part in (*startup_command, *rollback_command)):
            raise ValueError("startup and rollback commands must be direct arguments")
        if not independent_judge_id.strip() or type(sealed_at) is not int or sealed_at < 0:
...
def load_release_manifest(path: str | Path) -> tuple[CloseoutManifest, str]:
    ...
    try:
        ...
        for key, values in document["requirement_evidence"].items():
            evidence[key] = tuple(EvidenceRef(item["uri"], item["digest"], EvidenceKind(item["kind"]), item["subject_id"], item["observed_at"]) for item in values)
        obligations = tuple(ExternalObligation(item["obligation_id"], Disposition(item["kind"]), item["description"], tuple(item["blocks_claims"])) for item in document["obligations"])
        manifest = CloseoutManifest(
            document["release_id"], document["candidate_digest"], document["previous_release_digest"],
            evidence, {key: Disposition(value) for key, value in document["node_dispositions"].items()}, obligations,
            tuple(document["startup_command"]), tuple(document["rollback_command"]), document["independent_judge_id"], document["sealed_at"],
        )
        return manifest, supplied
    except (OSError, UnicodeError, json.JSONDecodeError, KeyError, TypeError, ValueError) as exc:
        if isinstance(exc, ValueError) and str(exc) == "release manifest digest mismatch":
            raise
