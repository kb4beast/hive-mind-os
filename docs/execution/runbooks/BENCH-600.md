...
- no-test-csharp: assert a `tests/expected_behavior.md` file created by the
  patch exists AND `Program.cs` is unchanged byte-for-byte vs a recorded
  sha256. **Do not bake that sha256 in as a literal.** `.gitattributes:2-4`
  pins `*.py`, `*.json` and `*.md` to LF but declares no rule for `*.cs` or
  `*.js`, and this repository sets `core.autocrlf=true`, so those fixture
  bytes are CRLF on a Windows checkout and LF on a POSIX one and a literal
  constant fails on the other platform. Record it as a
  `{{sha256:Program.cs}}` placeholder that `load_scenarios` resolves from the
  fixture as loaded; the assertion stays byte-exact.
...
6. Append every attempt record to `raw-results.jsonl` (canonical lines).
   ...
   **Zeroing `latency_seconds` alone is not sufficient** and two same-seed runs
   will still disagree: `metrics.json` embeds the measured latency, so the
   sha256 of that file recorded under the attempt's `artifacts` map drifts too.
   Measured: a field-by-field diff of two same-seed runs showed
   `artifacts["metrics.json"]` as the only differing field, on all 16 records.
   The digest input must therefore also restate that one hash at zero latency.
   Every other measured dimension stays digest-bound.
...
def _load(alias: str, filename: str):
    ...
    sys.modules[alias] = module   # REQUIRED before exec_module — see below
...
The `sys.modules[alias] = module` line is load-bearing, not decoration: any
module in this pair that defines a `@dataclass` crashes without it, because
`dataclasses._is_type` resolves the class's module via
`sys.modules.get(cls.__module__).__dict__`. Measured on the first execution —
`AttributeError: 'NoneType' object has no attribute '__dict__'` raised from
`dataclasses.py:814` while executing `comparator_court.py:82`. Registering the
alias first is still not a `sys.path` mutation and still creates no package.
...
| superiority-claim-guard-tests | `SuperiorityClaimGuardTests` | `test_guard_flags_superiority_phrasing` (build the probe inside the test by concatenation — `"Hive Mind OS " + "outper" + "forms the baseline on this benchmark"` — so the forbidden phrase never appears verbatim in any scanned repository file; guard returns nonempty. Note: `find_unauthorized_claims` scans `docs/**/*.md` including this runbook, which is why the probe must be assembled at test runtime); `test_guard_requires_disclaimer_line`; `test_results_document_passes_guard` (real `docs/benchmarks/HIVE_CORTEX_RESULTS.md` → `()`); `test_non_measurement_disposition_raises_superiority_claim_error`; `test_repo_docs_scan_stays_clean` (`hive_mind_os.benchmark_harness.find_unauthorized_claims(repo_root)` does not flag `docs/benchmarks/HIVE_CORTEX_RESULTS.md`) |
