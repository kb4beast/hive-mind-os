...
POST_MERGE_REPAIR_AUTHORITY_DOCUMENT = ".autopilot/post-merge-repair-authorities.json"
POST_MERGE_REPAIR_KIND = "hive-mind-autopilot-post-merge-repair-authorities-v1"
POST_MERGE_REPAIR_AUTHORITY_DIGEST = (
    "sha256:d09ada8bad4e967e18d19a303c15d52feca4fee5a0ba3b749dd2b77e7e7dfebd"
)
POST_MERGE_REPAIR_NODES = frozenset({"BUILDER-330", "ORCH-300", "OPTIMIZER-370"})
...
BUILDER_EVIDENCE_DIGEST = (
    "sha256:fa08e2cbd40acfe15b6d42c7005348b63b414d586814d8d4257abaffd7c66d06"
)
BUILDER_DIGESTS = {
...
BUILDER_RECOVERY_FIELDS = frozenset(
    {
        "schema_version",
        "kind",
        "recovery_id",
...
class SealedRecoveryMixin:
    ...
    def _post_merge_repair_issues(self) -> tuple[str, ...]:
        ...
        if not isinstance(raw_records, list) or len(raw_records) != 3:
            issues.append(
                "post-merge repair authority must contain exactly three records"
            )
            return tuple(dict.fromkeys(issues))
        ...
        for raw_record in raw_records:
            ...
            if (
                ...
                or any(
                    not isinstance(item, str) or item not in write_scope
            ...
            if (
                ...
                or set(claim_payload) != expected_claim_keys
            ):
            ...
            if target is not None:
                for label, commit in (
                    ...
                    if not self.is_ancestor(commit, target):
                        issues.append(
                            f"{node_id}: post-merge {label} is not integrated"
                        )
    ...
    def _repair_record(self, node_id: str) -> Mapping[str, Any]:
        ...
        if set(records) != set(REPAIR_AUTHORITY_MATERIAL_DIGESTS):
            raise AutopilotError(
                "sealed repair authority registry is incomplete or expanded"
            )
    ...
    def sealed_recovery_issues(self) -> tuple[str, ...]:
        ...
        if not isinstance(records, list) or len(records) != 2:
            issues.append(
                "sealed repair authority document must contain exactly two records"
            )
            return tuple(issues)
        observed: set[str] = set()
        for record in records:
            ...
            if digest_json(material) != REPAIR_AUTHORITY_MATERIAL_DIGESTS[str(node_id)]:
                issues.append(
                    f"{node_id}: sealed repair authority material was altered"
                )
            issues.extend(
                f"{node_id}: {issue}" for issue in self._capability_issues(capability)
            )
            node = super().node(str(node_id))
            if record.get("branch") != node.get("branch"):
                issues.append(
                    f"{node_id}: sealed repair branch differs from node contract"
                )
            ...
            if not isinstance(target, Mapping) or record.get(
                "repository"
            ...
            if not isinstance(allowed, list) or sorted(allowed) != expected_paths:
                issues.append(
                    f"{node_id}: sealed repair paths differ from exact node scope"
                )
            identities = [
            ...
            if (
                ...
                issues.append(
                    f"{node_id}: sealed court identities are missing or not distinct"
                )
    ...
    def _builder_record_issues(self) -> tuple[str, ...]:
        ...
        for key, expected in (
            ...
            (
                "appeals_ordering_disposition_digest",
                BUILDER_DIGESTS[BUILDER_APPEALS_PATH],
            ),
        ...
        if sorted(replan.get("preserved_candidate_paths", [])) != sorted(
    ...
    def _snapshot_rows(
        ...
        if not isinstance(rows, list):
            return []
        return [
            row for row in rows if isinstance(row, Mapping) and row.get(field) == value
        ]
    ...
    def _repair_live_issues(self, record: Mapping[str, Any]) -> tuple[str, ...]:
        ...
        if (
            self.target_requires_reconciliation()
            or self._reconciliation_digest() is None
        ):
            issues.append("sealed repair requires current singleton reconciliation")
        if self._snapshot_digest() is None:
            issues.append(
                "sealed repair requires a current authenticated GitHub snapshot"
            )
        ...
        if self.verify_git_objects:
            if not self.is_ancestor(incident, current):
                issues.append(
                    "sealed repair current release does not descend from incident target"
                )
            ...
            if changed & set(record["allowed_paths"]):
                issues.append(
                    "sealed repair scope changed between incident and execution release"
                )
        ...
        if len(prs) != 1:
            ...
            pr = prs[0]
            expected_ci = (
                "failure" if record["prior_state"] == "CI_FAILED" else "success"
            )
            expected = {
                "number": record["pr"],
                "state": "open",
            ...
            for key, expected_value in expected.items():
                if pr.get(key) != expected_value:
                    issues.append(
                        f"{node_id}: snapshot PR {key} is not the sealed value"
                    )
    ...
    def node_view(self, node_id: str) -> NodeView:
        ...
        if claim is not None:
            return NodeView(
                ...
                (),
                tuple(super().node(node_id).get("dependencies", [])),
                active_claim_owner=str(claim.get("owner"))
                if claim.get("owner")
                else None,
                branch=str(record["branch"]),
                pr_number=int(record["pr"]),
    ...
    @staticmethod
    def _claim_payload(
        record: Mapping[str, Any], claim: Mapping[str, Any]
    ) -> dict[str, Any]:
        return {
            "schema_version": 1,
    ...
    def _repair_claim_tree_and_parents(
        ...
        if merged.returncode != 0 or FULL_SHA.fullmatch(tree) is None:
            raise ClaimError(
                "ORCH sealed repair merge tree is conflicting or unavailable"
            )
        changed = self._diff_paths(target, tree)
        if tuple(sorted(changed)) != tuple(sorted(record["allowed_paths"])):
            raise ClaimError(
                "ORCH sealed repair merge tree differs from exact preserved scope"
            )
    ...
    def _create_repair_claim_commits(
        ...
        tree, parents = self._repair_claim_tree_and_parents(
            record, str(local["target_sha"])
        )
        ...
        args: list[str] = [
            "-c",
            "user.name=Hive Mind Autopilot Repair Claim",
            "-c",
            "user.email=autopilot-repair@hive-mind.invalid",
            "commit-tree",
            tree,
        ]
        ...
        created = self._git(
            tuple(args), check=True, environment=identity
        ).stdout.strip()
        if FULL_SHA.fullmatch(created) is None:
            raise ClaimError("sealed repair claim commit creation failed")
        if (
            self._commit_parents(created) != parents
            or self._commit_tree(created) != tree
        ):
        ...
        if merged.returncode != 0 or FULL_SHA.fullmatch(merge_tree) is None:
            raise ClaimError(
                "Optimizer sealed execution merge is conflicting or unavailable"
            )
        if tuple(sorted(self._diff_paths(target, merge_tree))) != tuple(
            sorted(record["allowed_paths"])
        ):
            raise ClaimError(
                "Optimizer execution merge differs from exact preserved scope"
            )
        merge_payload = {
            "schema_version": 1,
        ...
        merge_args = (
            "-c",
            "user.name=Hive Mind Autopilot Repair Merge",
            "-c",
            "user.email=autopilot-repair@hive-mind.invalid",
            "commit-tree",
            merge_tree,
        ...
        execution_merge = self._git(
            merge_args, check=True, environment=identity
        ).stdout.strip()
        if FULL_SHA.fullmatch(execution_merge) is None:
            raise ClaimError("Optimizer sealed execution merge creation failed")
        if (
            self._commit_parents(execution_merge) != (created, target)
            or self._commit_tree(execution_merge) != merge_tree
        ):
            raise ClaimError(
                "Optimizer sealed execution merge topology or tree is invalid"
            )
    ...
    def _fetch_exact_repair_head(self, record: Mapping[str, Any]) -> str:
        ...
        observed = self._git(
            ("rev-parse", "--verify", local_ref), check=False
        ).stdout.strip()
        if observed != old or not self.git_object_exists(old):
            raise ClaimError(
                "sealed repair fetched head differs from pinned old receipt"
            )
    ...
    def _old_repair_history_issues(self, record: Mapping[str, Any]) -> tuple[str, ...]:
        ...
        claim_target = str(
            record.get("claim_target_sha", record["incident_target_sha"])
        )
        issues: list[str] = []
        if self._commit_parents(old) != (candidate,):
            issues.append(
                "sealed old receipt is not directly parented by exact candidate"
            )
        if (
            self._commit_tree(candidate) != record["candidate_tree"]
            or self._commit_tree(old) != record["old_receipt_tree"]
        ):
        ...
        expected_claim_tree = record.get(
            "claim_target_tree", self._commit_tree(claim_target)
        )
        ...
        if tuple(sorted(self._diff_paths(claim_target, candidate))) != tuple(
            sorted(record["allowed_paths"])
        ):
            issues.append(
                "sealed historical candidate diff differs from exact node scope"
            )
        shown = self._git(("show", "-s", "--format=%B", old), check=False)
        receipt = (
            self._parse_receipt_message(shown.stdout) if shown.returncode == 0 else None
        )
        if (
            not isinstance(receipt, Mapping)
            or digest_json(receipt) != record["old_receipt_payload_digest"]
        ):
    ...
    def _release_binding_issues(
        self, local: Mapping[str, Any] | None = None
    ) -> tuple[str, ...]:
        issues: list[str] = []
        ...
        if release.get("target_sha") != target:
            issues.append(
                "sealed recovery release target differs from current singleton target"
            )
    ...
    def _live_release_issues(
        ...
        if not self._origin_is_configured_repository(record):
            return (
                "sealed recovery live release check requires literal configured origin",
            )
        ...
        if observed != expected:
            return (
                "literal origin singleton release differs from captured execution target",
            )
        ...
        try:
            ...
            if (
                ...
                or fetched_sha != expected
                or not self.git_object_exists(expected)
            ):
                return (
                    "literal origin singleton release object cannot be authenticated",
                )
    ...
    def _recover_interrupted_repair_claim(self, record: Mapping[str, Any]) -> None:
        ...
        if status not in {"PREPARING", "PREPARED"}:
            if (
                status == "CLAIMED"
                and parse_time(value.get("expires_at")) <= self.clock()
            ):
                ...
                if observed == record["old_receipt_commit"]:
                    append_jsonl(
                        ...
                        {
                            "event": "expired_claim_rolled_back",
                            "node_id": record["node_id"],
                ...
                retained["adverse_reason"] = (
                    "expired repair claim advanced beyond exact rollback head"
                )
        ...
        if status == "PREPARING" and created is None and observed == old:
            append_jsonl(
                self.state_dir / "sealed-repair-adverse.jsonl",
                {"event": "cleared_pre_cas_claim", "node_id": record["node_id"]},
            )
            path.unlink()
            return
        if (
            ...
            if observed == old:
                append_jsonl(
                    self.state_dir / "sealed-repair-adverse.jsonl",
                    {
                        "event": "rolled_back_interrupted_claim",
                        "node_id": record["node_id"],
    ...
    def _cas_update_branch(self, branch: str, expected: str, new: str) -> None:
        ...
        if pushed.returncode != 0:
            raise ClaimError(
                "sealed recovery compare-and-swap push failed: " + pushed.stderr.strip()
            )
        ...
        *,
        ...
        with self.claim_launch_authority_guard(
            ...
            with self.runtime_lock("claim-authority.lock", timeout_seconds=120.0):
                return self._claim_sealed_repair(
                    node_id,
                    owner,
                    claim_authority_class=claim_authority_class,
                    launch_instruction_id=launch_instruction_id,
    ...
    def _claim_sealed_repair(
        ...
        *,
        ...
        if not publish_remote or remote != "origin":
            raise ClaimError(
                "sealed repair claim requires literal origin remote publication"
            )
        ...
        if not self._origin_is_configured_repository(record):
            raise ClaimError(
                "sealed repair requires literal configured origin repository"
            )
        ...
        if not isinstance(release, Mapping):
            raise ClaimError(
                "sealed repair requires current explicit dispatcher release"
            )
        local: dict[str, Any] = {
            "schema_version": 1,
            "kind": REPAIR_CLAIM_KIND,
        ...
        local["claim_id"] = digest_json(local)
        ...
        try:
            os.write(
                descriptor,
                (json.dumps(local, sort_keys=True, indent=2) + "\n").encode(),
            )
            os.fsync(descriptor)
        finally:
            os.close(descriptor)
        ...
        try:
            ...
            if self.remote_branch_sha(branch) != old:
                raise ClaimError(
                    "sealed repair remote changed during claim verification"
                )
            if not self.is_ancestor(old, remote_head):
                raise ClaimError(
                    "sealed repair publication is not a fast-forward from old receipt"
                )
            ...
            if self._release_binding_issues(local):
                raise ClaimError(
                    "sealed repair release changed during claim publication"
                )
            ...
            if (
                ...
                and self.remote_branch_sha(branch) == remote_head
            ):
                try:
                    ...
                    append_jsonl(
                        ...
                        {
                            "node_id": node_id,
                            "event": "claim_compensation_failed",
                            "claim_commit": created,
                            "remote_head_commit": remote_head,
                        },
                    )
            ...
            if rolled_back:
                ...
                local["adverse_reason"] = (
                    "claim publication could not be rolled back or reconciled"
                )
                atomic_write_json(path, local)
            raise
        finally:
    ...
    def _release_sealed_repair(
        ...
        *,
        ...
        if value.get("status") == "COMPLETING":
            intent_path = (
                self.state_dir / f"sealed-repair-completion-{node_id.lower()}.json"
            )
            intent = read_json(intent_path) if intent_path.is_file() else None
            receipt_commit = (
                intent.get("receipt_commit") if isinstance(intent, Mapping) else None
            )
            if isinstance(receipt_commit, str):
                shown = self._git(
                    ("show", "-s", "--format=%B", receipt_commit), check=False
                )
                receipt = (
                    self._parse_receipt_message(shown.stdout)
        ...
        if isinstance(remote_head, str) and observed == remote_head:
            ...
            if (
                self.remote_branch_sha(str(record["branch"]))
                != record["old_receipt_commit"]
            ):
                raise ClaimError("sealed repair rollback verification failed")
        elif observed != record["old_receipt_commit"]:
            raise ClaimError(
                "sealed repair rollback is forbidden after the branch advanced"
            )
        append_jsonl(
            self.state_dir / "releases.jsonl",
            {
    ...
    def _sealed_receipt_shape_issues(
        ...
        for key in ("node_id", "branch", "timestamp", "rollback_ref"):
            if (
                ...
                or not str(receipt.get(key)).strip()
            ):
                issues.append(
                    f"sealed replacement receipt {key} must be a nonblank string"
                )
        ...
        if (
            ...
            issues.append(
                "sealed replacement receipt contract_version must be integer 1"
            )
        if (
            not isinstance(receipt.get("plan_fingerprint"), str)
            or DIGEST_SHA256.fullmatch(str(receipt.get("plan_fingerprint"))) is None
        ):
            issues.append(
                "sealed replacement receipt plan_fingerprint must be canonical SHA-256"
            )
        for key in ("base_commit", "base_tree", "final_commit", "final_tree"):
            if (
                not isinstance(receipt.get(key), str)
                or FULL_SHA.fullmatch(str(receipt.get(key))) is None
            ):
                issues.append(
                    f"sealed replacement receipt {key} must be a full lowercase Git SHA"
                )
        try:
            parsed_timestamp = parse_time(receipt.get("timestamp"))
        except (TypeError, ValueError):
            issues.append(
                "sealed replacement receipt timestamp must be canonical date-time text"
            )
        else:
            if receipt.get("timestamp") != format_time(parsed_timestamp):
                issues.append(
                    "sealed replacement receipt timestamp must be canonical UTC Z text"
                )
        ...
        for key in ("changed_paths", "evidence_refs"):
            ...
            if (
                ...
                issues.append(
                    f"sealed replacement receipt {key} must contain nonblank strings"
                )
        ...
        if not isinstance(roles, list) or not roles:
            ...
            for role in roles:
                if not isinstance(role, Mapping) or set(role) != {
                    ...
                    continue
                if any(
                    not isinstance(role.get(key), str) or not role.get(key, "").strip()
                    for key in role
                ):
                    issues.append(
                        "sealed replacement role identity fields must be nonblank"
                    )
                role_name = str(role.get("role"))
                if role_name in seen_roles:
                    issues.append(
                        "sealed replacement role identities contain a duplicate role"
                    )
                seen_roles.add(role_name)
                identity = str(role.get("identity"))
                if identity in seen_identities:
                    issues.append(
                        "sealed replacement role identities reuse an identity"
                    )
            ...
            if [
                role.get("role") for role in roles if isinstance(role, Mapping)
            ] != expected_roles:
                issues.append(
                    "sealed replacement role identity ordering differs from the node contract"
                )
        ...
        if not isinstance(tests, list) or not tests:
            ...
            if observed_test_names != list(self.node(node_id)["required_tests"]):
                issues.append(
                    "sealed replacement test ordering differs from the node contract"
                )
        authority = receipt.get("authority")
        authority_keys = {
            "node_id",
        ...
        if not isinstance(authority, Mapping) or set(authority) != authority_keys:
            issues.append(
                "sealed replacement authority shape is expanded or incomplete"
            )
        else:
            for key in ("node_id", "autonomy_level", "grant_id"):
                if (
                    not isinstance(authority.get(key), str)
                    or not authority.get(key, "").strip()
                ):
                    issues.append(
                        f"sealed replacement authority {key} must be nonblank text"
                    )
            ...
            if (
                ...
                issues.append(
                    "sealed replacement authority grants must contain one nonblank grant"
                )
            for key in (
                "supersedes_receipt_commit",
                "repair_claim_commit",
                ...
                if (
                    not isinstance(authority.get(key), str)
                    or FULL_SHA.fullmatch(str(authority.get(key))) is None
                ):
                    issues.append(
                        f"sealed replacement authority {key} must be a full lowercase Git SHA"
                    )
            for key in ("repair_authority_digest", "repair_claim_payload_digest"):
                if (
                    not isinstance(authority.get(key), str)
                    or DIGEST_SHA256.fullmatch(str(authority.get(key))) is None
                ):
                    issues.append(
                        f"sealed replacement authority {key} must be canonical SHA-256"
                    )
        ...
        if not isinstance(consultations, list):
            ...
            for consultation in consultations:
                if (
                    not isinstance(consultation, Mapping)
                    or set(consultation) != consultation_keys
                ):
                    issues.append(
                        "sealed replacement consultation shape is expanded or incomplete"
                    )
                    continue
                for key in (
                    "request_id",
                    ...
                    if (
                        not isinstance(consultation.get(key), str)
                        or not consultation.get(key, "").strip()
                    ):
                        issues.append(
                            f"sealed replacement consultation {key} must be nonblank text"
                        )
                request_ids.append(str(consultation.get("request_id")))
                if (
                    type(consultation.get("round")) is not int
                    or not 1 <= consultation.get("round", 0) <= 3
                ):
                    issues.append(
                        "sealed replacement consultation round must be integer 1 through 3"
                    )
                for key in (
                    "suspected_cheating",
                    "human_escalation",
                    ...
                    if type(consultation.get(key)) is not bool:
                        issues.append(
                            f"sealed replacement consultation {key} must be boolean"
                        )
                for key in ("evidence_refs", "dissent"):
                    ...
                    if not isinstance(values, list) or any(
                        ...
                        issues.append(
                            f"sealed replacement consultation {key} must contain nonblank strings"
                        )
                    elif len(values) != len(set(values)):
                        issues.append(
                            f"sealed replacement consultation {key} must be unique"
                        )
                ...
                if consultation.get("cheating_disposition") not in {
                    ...
                    issues.append(
                        "sealed replacement consultation cheating disposition is invalid"
                    )
                ...
                if answer is not None and (
                    ...
                    issues.append(
                        "sealed replacement consultation answer must be null or nonblank text"
                    )
                ...
                if authority_class is not None and (
                    ...
                    issues.append(
                        "sealed replacement consultation authority_class must be null or nonblank text"
                    )
                ...
                if (
                    ...
                    issues.append(
                        "sealed replacement consultation roles must contain two unique roles"
                    )
                    consulted = []
                if consultation.get("requesting_role") in consulted:
                    issues.append(
                        "sealed replacement consultation cannot consult the requesting role"
                    )
                identities = consultation.get("identity_records")
                if not isinstance(identities, list):
                    issues.append(
                        "sealed replacement consultation identities must be a list"
                    )
                    continue
                ...
                for identity in identities:
                    if not isinstance(identity, Mapping) or set(identity) != {
                        ...
                        issues.append(
                            "sealed replacement consultation identity shape is invalid"
                        )
                        continue
                    if any(
                        not isinstance(identity.get(key), str)
                        or not identity.get(key, "").strip()
                        for key in ("role", "identity", "identity_kind")
                    ):
                        issues.append(
                            "sealed replacement consultation identity fields must be nonblank strings"
                        )
                        continue
                    if identity.get("identity_kind") not in {
                        ...
                        issues.append(
                            "sealed replacement consultation identity kind is invalid"
                        )
                ...
                if len(identity_roles) != len(set(identity_roles)):
                    issues.append(
                        "sealed replacement consultation identities contain a duplicate role"
                    )
                if len(identity_values) != len(set(identity_values)):
                    issues.append(
                        "sealed replacement consultation identities reuse an identity"
                    )
                if identity_roles != consulted:
                    issues.append(
                        "sealed replacement consultation identities do not exactly order consulted roles"
                    )
            if len(request_ids) != len(set(request_ids)):
                issues.append(
                    "sealed replacement consultations contain a duplicate request_id"
                )
    ...
    def _replacement_receipt_issues(
        ...
        *,
        ...
        if not isinstance(authority, Mapping):
            return tuple(
                dict.fromkeys(
                    (*issues, "sealed replacement receipt authority is invalid")
                )
            )
        ...
        if (
            authority.get("node_id") != node_id
            or authority.get("autonomy_level") != "A3"
        ):
            issues.append(
                "sealed replacement authority node or autonomy level is invalid"
            )
        ...
        if (
            ...
            or FULL_SHA.fullmatch(claim_commit) is None
        ):
            issues.append("sealed replacement repair_claim_commit is invalid")
        execution_target = authority.get("execution_target_sha")
        if (
            not isinstance(execution_target, str)
            or FULL_SHA.fullmatch(execution_target) is None
        ):
        ...
        if (
            not isinstance(claim_payload_digest, str)
            or DIGEST_SHA256.fullmatch(claim_payload_digest) is None
        ):
            issues.append("sealed replacement repair_claim_payload_digest is invalid")
        expected_base = (
            record["incident_target_sha"]
            if node_id == "OPTIMIZER-370"
            else execution_target
        )
        if receipt.get("base_commit") != expected_base:
            issues.append(
                "sealed replacement base_commit differs from sealed provenance base"
            )
        if (
            self.verify_git_objects
            and isinstance(expected_base, str)
            and self.git_object_exists(expected_base)
        ):
            if receipt.get("base_tree") != self._commit_tree(expected_base):
                issues.append(
                    "sealed replacement base_tree differs from sealed provenance base"
                )
        ...
        if active_claim is not None:
            if claim_commit != active_claim.get("remote_claim_commit"):
                issues.append(
                    "sealed replacement repair claim differs from active lease"
                )
            if execution_target != active_claim.get("target_sha"):
                issues.append(
                    "sealed replacement execution target differs from active lease"
                )
            ...
            if authority.get("execution_merge_commit") != active_claim.get(
                "execution_merge_commit"
            ):
                issues.append(
                    "sealed replacement execution merge differs from active lease"
                )
        ...
        if (
            self.verify_git_objects
            and isinstance(claim_commit, str)
            and FULL_SHA.fullmatch(claim_commit)
        ):
            old = str(record["old_receipt_commit"])
                ...
                if not isinstance(message, Mapping):
                    ...
                    if set(message) != set(
                        self._claim_payload(
                            record,
                            {
                                ...
                                "github_snapshot_digest": message.get(
                                    "github_snapshot_digest"
                                ),
                                "reconciliation_digest": message.get(
                                    "reconciliation_digest"
                                ),
                                "doctor_evidence_digest": message.get(
                                    "doctor_evidence_digest"
                                ),
                            },
                        )
                    ...
                    for key, expected in {
                        ...
                        if message.get(key) != expected:
                            issues.append(
                                f"sealed repair claim message {key} is invalid"
                            )
                    if any(
                        not isinstance(message.get(key), str) or not message.get(key)
                        for key in (
                            "owner",
                            "expires_at",
                            "release_id",
                        ...
                        issues.append(
                            "sealed repair claim message dynamic evidence is incomplete"
                        )
                    if authority.get("repair_claim_payload_digest") != digest_json(
                        message
                    ):
                        issues.append(
                            "sealed replacement does not bind the complete repair claim payload"
                        )
                    if active_claim is not None and dict(
                        message
                    ) != self._claim_payload(record, active_claim):
                        issues.append(
                            "sealed repair claim message differs from active lease"
                        )
                if node_id == "OPTIMIZER-370":
                    if self._commit_tree(claim_commit) != record["old_receipt_tree"]:
                        issues.append("Optimizer repair claim is not zero-path")
                else:
                    tree, _ = self._repair_claim_tree_and_parents(
                        record, str(execution_target)
                    )
                    if self._commit_tree(claim_commit) != tree:
                        issues.append(
                            "ORCH repair merge claim tree is not deterministic"
                        )
            execution_merge = authority.get("execution_merge_commit")
            if (
                not isinstance(execution_merge, str)
                or FULL_SHA.fullmatch(execution_merge) is None
            ):
                issues.append("sealed replacement execution_merge_commit is invalid")
            elif node_id == "ORCH-300":
                if execution_merge != claim_commit:
                    issues.append(
                        "ORCH execution merge must be the repair claim itself"
                    )
            elif not self.git_object_exists(execution_merge):
                issues.append("Optimizer execution merge object is unavailable")
            else:
                expected_merge = self._git(
                    ("merge-tree", "--write-tree", claim_commit, str(execution_target)),
                    check=False,
                )
                merge_tree = expected_merge.stdout.strip()
                if (
                    expected_merge.returncode != 0
                    or FULL_SHA.fullmatch(merge_tree) is None
                ):
                    issues.append("Optimizer execution merge tree cannot be reproduced")
                if self._commit_parents(execution_merge) != (
                    claim_commit,
                    execution_target,
                ):
                    issues.append(
                        "Optimizer execution merge parent ordering is invalid"
                    )
                if self._commit_tree(execution_merge) != merge_tree:
                    issues.append("Optimizer execution merge tree is not deterministic")
                if tuple(
                    sorted(self._diff_paths(str(execution_target), execution_merge))
                ) != tuple(sorted(record["allowed_paths"])):
                    issues.append(
                        "Optimizer execution merge differs from exact preserved scope"
                    )
            if (
                not isinstance(final, str)
                or not isinstance(execution_merge, str)
                or not self.is_ancestor(execution_merge, final)
            ):
                issues.append(
                    "sealed replacement final does not descend from repair claim"
                )
            if (
                not isinstance(execution_target, str)
                or not isinstance(final, str)
                or not self.is_ancestor(execution_target, final)
            ):
                issues.append(
                    "sealed replacement final does not descend from execution release"
                )
            if isinstance(execution_target, str) and isinstance(final, str):
                ...
                if (
                    ...
                    or tuple(sorted(declared)) != observed
                ):
                    issues.append(
                        "sealed replacement changed_paths differ from execution release diff"
                    )
                if not observed or set(observed) - set(record["allowed_paths"]):
                    issues.append(
                        "sealed replacement diff expands or omits exact repair scope"
                    )
        return tuple(dict.fromkeys(issues))
    ...
    def _claim_provenance_issues(
        ...
        try:
            original_payload = (
                json.loads(shown.stdout.strip()) if shown.returncode == 0 else None
            )
        except json.JSONDecodeError:
        ...
        if not isinstance(original_payload, Mapping) or any(
            original_payload.get(key) != value for key, value in expected.items()
        ):
    ...
    def validate_receipt(
        ...
        *,
        ...
        if node_id not in REPAIR_AUTHORITY_MATERIAL_DIGESTS or not isinstance(
            value, Mapping
        ):
            return super().validate_receipt(
                node_id, value, require_integrated=require_integrated
            )
        authority = value.get("authority")
        if (
            not isinstance(authority, Mapping)
            or "supersedes_receipt_commit" not in authority
        ):
            return super().validate_receipt(
                node_id, value, require_integrated=require_integrated
            )
        ...
        if (
            require_integrated
            and self.verify_git_objects
            and isinstance(final, str)
            and not self.is_ancestor(final, self.current_target_sha())
        ):
    ...
    def resolve_sealed_repair_records(
        ...
        old = next(
            (
                item
                for item in records
                if item.get("commit") == record["old_receipt_commit"]
            ),
            None,
        )
        replacement = next(
    ...
    def _receipt_index_contains(self, node_id: str, receipt_commit: str) -> bool:
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            ...
            if (
                ...
                and value.get("receipt_commit") == receipt_commit
            ):
                return True
        return False
    ...
    def _recover_interrupted_repair_completion(
        ...
        if claim.get("status") != "COMPLETING" or intent.get("target_sha") != claim.get(
            "target_sha"
        ):
            raise ClaimError(
                "sealed repair completion recovery target or state is invalid"
            )
        ...
        def adverse(error: Exception) -> None:
            ...
            append_jsonl(
                self.state_dir / "sealed-repair-adverse.jsonl",
                {
                    "event": "receipt_restart_recovery_failed",
                    "node_id": node_id,
                    "remote_head": remote,
        ...
        if receipt_commit is None:
            if remote != final:
                error = ClaimError(
                    "sealed repair completion stopped before commit but remote moved"
                )
                adverse(error)
                raise error
            ...
            if local != final:
                if (
                    ...
                    error = ReceiptError(
                        "sealed repair interrupted local receipt is ambiguous"
                    )
                    adverse(error)
                    raise error
                ...
                if not isinstance(local_receipt, Mapping) or digest_json(
                    local_receipt
                ) != digest_json(receipt):
                    error = ReceiptError(
                        "sealed repair interrupted local receipt payload is invalid"
                    )
                    adverse(error)
                    raise error
                rolled_back = self._git(
                    ("update-ref", local_ref, final, local), check=False
                )
                observed = self._git(
                    ("rev-parse", "--verify", local_ref), check=False
                ).stdout.strip()
                if rolled_back.returncode != 0 or observed != final:
                    error = ReceiptError(
                        "sealed repair interrupted local receipt rollback failed"
                    )
                    adverse(error)
                    raise error
            restored = dict(claim)
        ...
        if (
            ...
            error = ClaimError(
                "sealed repair completion intent receipt commit is invalid"
            )
            adverse(error)
            raise error
        if remote == final:
            ...
            if local == receipt_commit:
                updated = self._git(
                    ("update-ref", local_ref, final, receipt_commit), check=False
                )
                local = self._git(
                    ("rev-parse", "--verify", local_ref), check=False
                ).stdout.strip()
        ...
        if remote != receipt_commit:
            error = ClaimError(
                "sealed repair completion recovery remote head is ambiguous"
            )
            adverse(error)
            raise error
        if self._commit_parents(receipt_commit) != (final,) or self._commit_tree(
            receipt_commit
        ) != receipt.get("final_tree"):
            error = ReceiptError(
                "sealed repair recovered receipt topology or tree is invalid"
            )
            adverse(error)
            raise error
        ...
        if not isinstance(recovered_receipt, Mapping) or digest_json(
            recovered_receipt
        ) != digest_json(receipt):
        ...
        validation_lease = self.acquire_global_validation_lease_internal(
            node_id, validation_owner, lease_minutes=10
        )
        try:
            ...
            if local_path.is_file():
                if digest_json(read_json(local_path)) != digest_json(receipt):
                    raise ReceiptError(
                        "sealed repair recovered local receipt conflicts"
                    )
            else:
            ...
            self.release_global_validation_lease_internal(
                node_id,
                validation_owner,
                lease_id=str(validation_lease["lease_id"]),
            )
    ...
    def complete(
        ...
        *,
        ...
        with self.claim_launch_authority_guard(
            ...
            with self.runtime_lock("claim-authority.lock", timeout_seconds=120.0):
                return self._complete_sealed_repair(
                    node_id,
                    owner,
                    receipt,
                    claim_id=claim_id,
    ...
    def _complete_sealed_repair(
        ...
        *,
        ...
        if not self._origin_is_configured_repository(record):
            raise ClaimError(
                "sealed repair completion requires literal configured origin"
            )
        ...
        if (
            not isinstance(final, str)
            or self.remote_branch_sha(str(record["branch"])) != final
        ):
            raise ReceiptError(
                "sealed repair remote branch must equal the exact final candidate"
            )
        ...
        try:
            if self._release_binding_issues(claim):
                raise ClaimError(
                    "sealed repair release changed before receipt publication"
                )
            if self.remote_branch_sha(str(record["branch"])) != final:
                raise ReceiptError(
                    "sealed repair remote final moved before receipt publication"
                )
            intent = {
                "schema_version": 1,
            ...
            remote_published = (
                self.remote_branch_sha(str(record["branch"])) == receipt_commit
            )
            if not remote_published:
                raise ReceiptError(
                    "sealed repair receipt remote CAS verification failed"
                )
            if self._release_binding_issues(claim):
                raise ClaimError(
                    "sealed repair release changed during receipt publication"
                )
            ...
            if (
                ...
                and self.remote_branch_sha(str(record["branch"])) == receipt_commit
            ):
                try:
                    self._cas_update_branch(
                        str(record["branch"]), receipt_commit, str(final)
                    )
            ...
            if receipt_commit is not None:
                local_ref = f"refs/heads/{record['branch']}"
                local_update = self._git(
                    ("update-ref", local_ref, str(final), receipt_commit), check=False
                )
            ...
            if compensated and local_rolled_back:
                ...
                claim_state["adverse_reason"] = (
                    "receipt publication could not be rolled back"
                )
                atomic_write_json(claim_path, claim_state)
                append_jsonl(
                    self.state_dir / "sealed-repair-adverse.jsonl",
                    {
                        "event": "receipt_compensation_failed",
                        "node_id": node_id,
                        "receipt_commit": receipt_commit,
                        "final_commit": final,
                    },
    ...
    def _builder_execution(self) -> Mapping[str, Any] | None:
        ...
        if (
            value.get("kind") != BUILDER_EXECUTION_KIND
            or value.get("status") != "RETIRED"
        ):
            raise AutopilotError("Builder retirement execution disposition is invalid")
        return value

    def _builder_recovery(self) -> Mapping[str, Any] | None:
        path = self.builder_recovery_path
        if not path.exists():
            return None
        path = _reject_link_components(
            path, label="Builder retirement recovery authority"
        )
        if not path.is_file():
            raise AutopilotError("Builder retirement recovery authority is not a file")
        value = read_strict_canonical_json(
            path,
            label="Builder retirement recovery authority",
            expected_fields=set(BUILDER_RECOVERY_FIELDS),
        )
        material = dict(value)
        record_id = material.pop("record_id", None)
        if (
            value.get("schema_version") != 1
            or value.get("kind") != BUILDER_EXECUTION_KIND
            or value.get("recovery_id") != "builder-330-stale-candidate-recovery-v1"
            or record_id != digest_json(material)
            or DIGEST_SHA256.fullmatch(str(record_id)) is None
            or DIGEST_SHA256.fullmatch(str(value.get("snapshot_digest"))) is None
            or any(
                item is not None and DIGEST_SHA256.fullmatch(str(item)) is None
                for item in (
                    value.get("reconciliation_digest"),
                    value.get("doctor_evidence_digest"),
                )
            )
            or FULL_SHA.fullmatch(str(value.get("target_sha"))) is None
        ):
            raise AutopilotError("Builder retirement recovery authority is invalid")
        parse_time(value.get("recorded_at"))
        return value

    def _write_builder_recovery(self, material: Mapping[str, Any]) -> None:
        if "record_id" in material:
            raise AutopilotError("Builder retirement recovery material is ambiguous")
        atomic_write_json(
    ...
    def _builder_recovery_issues(self) -> tuple[str, ...]:
        ...
        if execution is None:
            return ()
        recovery = self._builder_recovery()
        if recovery is None:
            return (
                "Builder retirement requires fresh snapshot, reconciliation, and doctor before dispatch",
            )
        ...
        if branches:
            return (
                "Builder retirement fresh snapshot still contains canonical source",
            )
    ...
    def after_reconcile(self) -> None:
        if (
            self._builder_execution() is None
            or not self.builder_recovery_path.is_file()
        ):
            return
        recovery = self._builder_recovery()
        if (
            recovery is None
            or recovery.get("snapshot_digest") != self._snapshot_digest()
        ):
            return
        ...
        self._write_builder_recovery(updated)
    ...
    def after_doctor(self) -> None:
        ...
        if recovery is None:
            return
        if (
            recovery.get("snapshot_digest") != self._snapshot_digest()
            or recovery.get("reconciliation_digest") != self._reconciliation_digest()
        ):
            return
        updated = dict(recovery)
        updated.pop("record_id", None)
    ...
    def _builder_history_issues(self, replan: Mapping[str, Any]) -> tuple[str, ...]:
        ...
        if (
            self._commit_tree(claim) != replan["claim_tree"]
            or self._commit_tree(candidate) != replan["candidate_tree"]
        ):
            return ("Builder sealed claim or candidate tree differs",)
        if tuple(
            sorted(self._diff_paths(str(replan["claim_target_sha"]), candidate))
        ) != tuple(sorted(replan["preserved_candidate_paths"])):
    ...
    def _recover_expired_builder_lease(self, replan: Mapping[str, Any]) -> None:
        ...
        if not isinstance(value, Mapping) or set(value) != required:
            raise ClaimError(
                "Builder retirement existing lease is foreign or malformed"
            )
        if (
            value.get("kind") != BUILDER_EXECUTION_KIND
            ...
            raise ClaimError(
                "Builder retirement existing lease requires reconciliation"
            )
        ...
        if (
            self._remote_ref_sha(source_ref) != replan["expected_remote_head"]
            or self._remote_ref_sha(str(replan["archive_ref"])) is not None
        ):
            raise ClaimError(
                "expired Builder lease cannot be recovered after ref mutation"
            )
        archive = (
            self.state_dir
            / "builder-retirement-leases"
            / (digest_json(value).replace(":", "-") + ".json")
        )
        atomic_write_json(
            archive,
            {
                **value,
                ...
                "recovered_at": format_time(self.clock()),
            },
        )
    ...
    def _recover_interrupted_builder_retirement(
        ...
        if not isinstance(intent, Mapping) or set(intent) != required:
            raise ClaimError(
                "Builder retirement prepared intent is foreign or malformed"
            )
        if (
            intent.get("kind") != BUILDER_EXECUTION_KIND
            or intent.get("status") != "PREPARED"
        ):
            raise ClaimError(
                "Builder retirement prepared intent requires reconciliation"
            )
        if not self.builder_lease_path.is_file():
            raise ClaimError("Builder interrupted retirement lease is missing")
        lease = read_json(self.builder_lease_path)
        if not isinstance(lease, Mapping) or digest_json(lease) != intent.get(
            "lease_digest"
        ):
        ...
        if source == expected and archive is None:
            if (
                self.builder_lease_path.is_file()
                and parse_time(read_json(self.builder_lease_path).get("expires_at"))
                <= self.clock()
            ):
                self._recover_expired_builder_lease(replan)
                self.builder_intent_path.unlink()
                append_jsonl(
                    self.state_dir / BUILDER_AUDIT_FILE,
                    {
        ...
        if source is not None or archive != expected:
            raise ClaimError(
                "Builder retirement prepared transaction refs are ambiguous"
            )
        fetch_ref = "refs/hive-mind-autopilot/repair-fetch/builder-330/" + expected
        self._git(("update-ref", "-d", fetch_ref), check=False)
        fetched = self._git(
            ("fetch", "--no-tags", "origin", f"{archive_ref}:{fetch_ref}"), check=False
        )
        try:
            if fetched.returncode != 0 or self._builder_history_issues(replan):
                raise ClaimError(
                    "Builder interrupted retirement archive verification failed"
                )
            ...
            append_jsonl(
                self.state_dir / BUILDER_AUDIT_FILE,
                {"event": "builder_branch_retirement_recovered", **execution},
            )
    ...
    def _builder_release_preflight(self) -> Mapping[str, Any]:
        ...
        if not isinstance(release, Mapping) or self._release_issues(release):
            raise ClaimError(
                "Builder retirement requires one exact current dispatcher release"
            )
        ...
        if not isinstance(verdicts, Mapping) or verdicts.get("BUILDER-330") != "STOP":
            raise ClaimError(
                "Builder retirement requires an exact Builder STOP verdict"
            )
        view = super().node_view("BUILDER-330")
        if view.state != "REPAIR_REQUIRED":
            raise ClaimError(
                "Builder retirement requires exact controller REPAIR_REQUIRED state"
            )
        return release

    def _compensate_builder_retirement(
        self, source_ref: str, archive_ref: str, expected: str
    ) -> bool:
        if (
            self._remote_ref_sha(source_ref) is not None
            or self._remote_ref_sha(archive_ref) != expected
        ):
            return False
        pushed = self._git(
            (
                "push",
                "--atomic",
                f"--force-with-lease={source_ref}:",
                f"--force-with-lease={archive_ref}:{expected}",
                "origin",
                f"{expected}:{source_ref}",
                f":{archive_ref}",
            ),
            check=False,
        )
    ...
    def retire_builder_branch(self, *, actor: str) -> Mapping[str, Any]:
        ...
        if not self._origin_is_configured_repository(replan):
            raise ClaimError(
                "Builder retirement requires literal origin repository identity"
            )
        ...
        if (
            self.target_requires_reconciliation()
            or self._snapshot_digest() is None
            or self._reconciliation_digest() is None
        ):
            raise ClaimError(
                "Builder retirement requires current snapshot and reconciliation"
            )
        if self._doctor_evidence_digest() is None:
        ...
        if self.verify_git_objects and not self.is_ancestor(
            capability, self.current_target_sha()
        ):
        ...
        if (
            len(branches) != 1
            or branches[0].get("sha") != replan["candidate_commit"]
            or branches[0].get("stale") is not True
        ):
            raise ClaimError(
                "Builder retirement snapshot does not bind exact stale branch"
            )
        ...
        if (
            self.claim_path("BUILDER-330").is_file()
            or self.validation_lease_path.is_file()
        ):
            raise ClaimError(
                "Builder retirement is forbidden by active claim or validation lease"
            )
        ...
        try:
            descriptor = os.open(
                self.builder_lease_path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600
            )
        except FileExistsError as error:
        ...
        fetch_ref = "refs/hive-mind-autopilot/repair-fetch/builder-330/" + str(
            replan["expected_remote_head"]
        )
        try:
            source_ref = f"refs/heads/{replan['branch']}"
            archive_ref = str(replan["archive_ref"])
            ...
            if self._remote_ref_sha(archive_ref) is not None:
                raise ClaimError(
                    "Builder retirement archive already exists; reuse is forbidden"
                )
            if (
                parse_time(read_json(self.builder_lease_path).get("expires_at"))
                <= self.clock()
            ):
                raise ClaimError("Builder retirement lease expired")
            self._git(("update-ref", "-d", fetch_ref), check=False)
            fetched = self._git(
                ("fetch", "--no-tags", "origin", f"{source_ref}:{fetch_ref}"),
                check=False,
            )
            observed = self._git(
                ("rev-parse", "--verify", fetch_ref), check=False
            ).stdout.strip()
            if fetched.returncode != 0 or observed != expected:
                raise ClaimError(
                    "Builder retirement cannot fetch the exact source object"
                )
            history_issues = self._builder_history_issues(replan)
            if history_issues:
                raise ClaimError("; ".join(history_issues))
            if (
                self._remote_ref_sha(source_ref) != expected
                or self._remote_ref_sha(archive_ref) is not None
            ):
                raise ClaimError("Builder retirement refs changed during verification")
            if self._release_binding_issues():
                raise ClaimError(
                    "Builder retirement release changed before archive transaction"
                )
            ...
            pushed = self._git(
                (
                    "push",
                    "--atomic",
                    f"--force-with-lease={source_ref}:{expected}",
                    f"--force-with-lease={archive_ref}:",
                    "origin",
                    f"{expected}:{archive_ref}",
                    f":{source_ref}",
                ),
                check=False,
            )
            if pushed.returncode != 0:
                raise ClaimError(
                    "Builder retirement atomic archive/delete failed: "
                    + pushed.stderr.strip()
                )
            try:
                if (
                ...
                fetched = self._git(
                    ("fetch", "--no-tags", "origin", f"{archive_ref}:{fetch_ref}"),
                    check=False,
                )
                if (
                    fetched.returncode != 0
                    or self._commit_tree(expected) != replan["candidate_tree"]
                    or not self.is_ancestor(str(replan["claim_commit"]), expected)
                ):
                    raise ClaimError(
                        "Builder retirement archive tree or ancestry verification failed"
                    )
                if self._release_binding_issues():
                    raise ClaimError(
                        "Builder retirement release changed during archive transaction"
                    )
                live_issues = self._live_release_issues(
                    replan, str(lease["target_sha"])
                )
                if live_issues:
                    raise ClaimError("; ".join(live_issues))
                execution = {
                ...
                append_jsonl(
                    self.state_dir / BUILDER_AUDIT_FILE,
                    {"event": "builder_branch_retired", **execution},
                )
                self.builder_intent_path.unlink(missing_ok=True)
                return execution
            except Exception as error:
                compensated = self._compensate_builder_retirement(
                    source_ref, archive_ref, expected
                )
                append_jsonl(
                    self.state_dir / BUILDER_AUDIT_FILE,
                    {
                        "event": "builder_retirement_compensated"
                        if compensated
                        else "builder_retirement_compensation_failed",
                        "source_head": expected,
                        "archive_ref": archive_ref,
                        "error": str(error),
                        "recorded_at": format_time(self.clock()),
