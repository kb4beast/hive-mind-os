...
class DeliveryRequest:
    ...
    def __post_init__(self):
        if not all((self.request_id,self.package_id,self.tenant_id,self.candidate_commit,self.target,self.base,self.run_branch,self.idempotency_key)): raise ValueError("delivery identity is required")
        if self.run_branch in {self.base,"main","master"}: raise ValueError("protected branch cannot be a run branch")
@dataclass(frozen=True,slots=True)
...
class DeliveryBroker:
    ...
    def prepare(self,r:DeliveryRequest)->DeliveryResult:
        ...
        if not r.qualification_digest.startswith("sha256:"): return DeliveryResult(DeliveryState.BLOCKED,digest,blocker="qualification receipt is not authenticated")
        if any(".." in p or p.startswith("/") for p in r.changed_paths): return DeliveryResult(DeliveryState.BLOCKED,digest,blocker="invalid changed path")
        if re.search(r"(token|secret|password|api[_-]?key)\s*[:=]", r.content_digest, re.I): return DeliveryResult(DeliveryState.BLOCKED,digest,blocker="secret-bearing content")
