...
class FeedbackObservation:
    ...
    def __post_init__(self):
        if not all((self.provider,self.pr_id,self.head,self.event_id,self.source_identity,self.content_digest)): raise ValueError("feedback identity is required")
@dataclass(frozen=True,slots=True)
...
class FeedbackObserver:
    ...
    def snapshot(self): return {"cursor":self.cursor,"seen":tuple(sorted(self.seen)),"repairs":tuple(self.repairs)}
    def restore(self,state):
        self.cursor=state.get("cursor"); self.seen=set(tuple(x) for x in state.get("seen",()));
        self.repairs={}
