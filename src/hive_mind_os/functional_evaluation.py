...
class FunctionalEvaluation:
 ...
 def __post_init__(self):
  ...
  if self.reference_health is CheckOutcome.UNAVAILABLE and self.verdict is EvaluationVerdict.PASS: raise ValueError("unavailable reference cannot pass")
 @property
 def digest(self): return canonical_digest(self)
 def to_dict(self): return canonical_document(self)
 @classmethod
 def from_dict(cls,value):
  fields={"episode_id","candidate_digest","rubric_digest","environment_digest","reference_health","required_checks","capability_results","quality_metrics","security_results","resource_use","contamination_status","evaluator_id","verdict","evidence_refs"}
  if set(value)!=fields: raise ValueError("closed evaluation schema")
  return cls(**{**value,"reference_health":CheckOutcome(value["reference_health"]),"verdict":EvaluationVerdict(value["verdict"]),"required_checks":tuple(value["required_checks"]),"evidence_refs":tuple(value["evidence_refs"]),"capability_results":{k:CheckOutcome(v) for k,v in value["capability_results"].items()},"security_results":{k:CheckOutcome(v) for k,v in value["security_results"].items()}})

class FunctionalEvaluationStore:
 def __init__(self,path): self._store=ContractStore(path,FunctionalEvaluation.from_dict)
 def put(self,evaluation): return self._store.put(evaluation.digest,evaluation)
...
class FunctionalAdapter:
 ...
 def evaluate(self, reference, candidate, checks):
  ...
  for name, check in checks.items():
   try: results[name]=CheckOutcome.PASS if check(reference,candidate) else CheckOutcome.FAIL
   except NotImplementedError: results[name]=CheckOutcome.UNAVAILABLE
   except Exception: results[name]=CheckOutcome.INCONCLUSIVE
  return results
