...
class QualificationRequest:
    ...
    def __post_init__(self):
        if not self.candidate_id or not self.base_id or not self.acceptance_manifest or not self.evaluator_id: raise ValueError("candidate, base, acceptance and evaluator are required")
@dataclass(frozen=True,slots=True)
class CheckReceipt:
...
class CandidateQualifier:
    ...
    def qualify(self, request:QualificationRequest)->QualificationReceipt:
        ...
        if self.executor is None:
            return QualificationReceipt(canonical_digest(request),QualificationDisposition.INCOMPLETE,(),("toolchain-unexecuted",))
        for name in request.acceptance_manifest:
            ...
            if not isinstance(count,int) or count < 0: return QualificationReceipt(canonical_digest(request),QualificationDisposition.QUARANTINED,tuple(checks),("invalid-executor-result",))
    ...
    @staticmethod
    def invalidated(receipt:QualificationReceipt, request:QualificationRequest, *, candidate_id:str|None=None, environment_digest:str|None=None)->tuple[str,...]:
        reasons=list(receipt.invalidations)
        if candidate_id is not None and candidate_id != request.candidate_id: reasons.append("candidate-changed")
        if environment_digest is not None and environment_digest != request.environment_digest: reasons.append("environment-changed")
