...
class RobloxRuntimeEvidence:
    ...
    def __post_init__(self):
        if (
            ...
            and self.missing_obligations
        ):
            raise ValueError("production candidate cannot have missing obligations")
        if (
            self.verdict
            in {RuntimeVerdict.RUNTIME_VALIDATED, RuntimeVerdict.PRODUCTION_CANDIDATE}
            and not self.runtime_receipts
        ):
            raise ValueError("runtime verdict requires receipts")
        if any(
            not isinstance(x, str) or not x.startswith("ref:")
            for x in self.account_capability_refs
        ):
            raise ValueError("capabilities must be opaque references")
        ...
        return cls(
            **{
                **value,
                ...
                "account_capability_refs": tuple(value["account_capability_refs"]),
                "runtime_receipts": tuple(value["runtime_receipts"]),
                "performance_samples": tuple(value["performance_samples"]),
                "missing_obligations": tuple(value["missing_obligations"]),
            }
        )


def blocked_runtime(reason):
