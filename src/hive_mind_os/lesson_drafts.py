...
class LessonDraft:
 ...
 def __post_init__(self):
  if self.document_status!="DRAFT": raise DraftError("document_status must be DRAFT")
  ...
  req=(self.draft_id,self.origin_subject_token,self.origin_mission_token,self.outcome_class,self.private_custody_token,self.confidence_basis,self.export_policy_digest,self.generator_id,self.created_at)
  if any(type(v) is not str or not v.strip() for v in req): raise DraftError("required draft field missing")
  if any(type(v) is not str or not v.strip() for seq in (self.applicability,self.evidence_tokens,self.counterexamples,self.disposition_refs) for v in seq): raise DraftError("draft tokens must be non-empty")
  if self.processing_state is ProcessingState.REVIEWED_DRAFT and (not self.reviewer_id or not self.sanitization_receipt_digest): raise DraftError("review evidence required")
 @property
 def digest(self): return canonical_digest(self)
 def to_dict(self): return canonical_document(self)
 @classmethod
 def from_dict(cls,value):
  fields={"draft_id","document_status","processing_state","origin_subject_token","origin_mission_token","statement","applicability","outcome_class","evidence_tokens","private_custody_token","confidence_basis","counterexamples","disposition_refs","generator_id","reviewer_id","sanitization_receipt_digest","export_policy_digest","withheld_reason_code","created_at"}
  if set(value)!=fields: raise DraftError("closed draft schema")
  return cls(**{**value,"processing_state":ProcessingState(value["processing_state"]),"applicability":tuple(value["applicability"]),"evidence_tokens":tuple(value["evidence_tokens"]),"counterexamples":tuple(value["counterexamples"]),"disposition_refs":tuple(value["disposition_refs"])})
 def transition(self,state,**changes):
  if not isinstance(state,ProcessingState): raise DraftError("invalid processing state")
  if state is ProcessingState.SANITIZED_DRAFT and not changes.get("sanitization_receipt_digest",self.sanitization_receipt_digest): raise DraftError("sanitization receipt required")
...
class LessonDraftStore:
 def __init__(self,path): self._store=ContractStore(path,LessonDraft.from_dict)
 def put(self,draft): return self._store.put(draft.digest,draft)
 def get(self,digest): return self._store.get(digest)
 def reviewed_for_export(self,digest):
  draft=self.get(digest)
  if draft is None or draft.processing_state not in {ProcessingState.REVIEWED_DRAFT,ProcessingState.COMMITTED_DRAFT,ProcessingState.DRAFT_PR_OPEN}: raise DraftError("draft is not exportable")
  return draft
