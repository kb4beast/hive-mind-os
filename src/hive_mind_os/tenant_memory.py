...
class SubjectMemoryBoundary:
    ...
    def from_dict(cls, value):
        required={"tenant_id","repository_id","private_store_id","cache_partition_id","logging_partition_id","provider_policy_digest","encryption_profile_id","retention_policy_digest","state"}
        if set(value)!=required: raise BoundaryError("closed boundary schema")
...
class SubjectNamespace:
    def __init__(self, path=None):
        self._data:dict[tuple[str,str,str],Any]={}; self._states:dict[str,BoundaryState]={}
        self._boundaries=ContractStore(path, SubjectMemoryBoundary.from_dict) if path else None
    ...
    def put(self,boundary:SubjectMemoryBoundary,name:str,value:Any):
        ...
        if type(name) is not str or not name or name.startswith("/") or ".." in name.split("/"): raise BoundaryError("invalid namespace key")
        self._data[self.key(boundary,name)]=value
        if self._boundaries: self._boundaries.put(boundary.digest, boundary)
