...
class ModelRoute:
    ...
    def __post_init__(self):
        ...
        if not self.capability_ids or len(set(self.capability_ids)) != len(self.capability_ids): raise ValueError("route must have unique capabilities")
        if any(not isinstance(x, str) or not x for x in self.capability_ids): raise ValueError("route capabilities must be identifiers")
        if min(self.max_calls,self.max_tokens)<0 or any(isinstance(x, bool) for x in (self.max_calls,self.max_tokens)): raise ValueError("route must have nonnegative integer limits")
    @property
...
def select_route(*, required_capabilities: tuple[str,...], permitted_policy_digest: str, routes: tuple[ModelRoute,...], deterministic: bool=False) -> RouteDecision:
    ...
    if any(not isinstance(x, str) or not x for x in required_capabilities): return RouteDecision(RouteStatus.BLOCKED_CAPABILITY,None,"invalid required capability")
...
class RepairLease:
    ...
    def consume(self, *, calls:int, tokens:int, repeated_failure:bool=False, route: ModelRoute | None = None) -> "RepairLease":
        if any(type(x) is not int or x < 0 for x in (calls, tokens)) or (repeated_failure and self.repair_attempts>=self.max_repairs): raise ValueError("repair lease exhausted or invalid")
        if route is not None and (self.used_calls + calls > route.max_calls or self.used_tokens + tokens > route.max_tokens):
...
class RouteUsageReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if any(type(x) is not int or x < 0 for x in (self.calls, self.tokens)) or not isinstance(self.status, RouteStatus) or not self.reason.strip():
            raise ValueError("invalid route usage receipt")
    @property
    def digest(self) -> str:
        return canonical_digest({"route_digest": self.route_digest, "calls": self.calls, "tokens": self.tokens, "status": self.status.value, "reason": self.reason})
...
class RouteEscalation:
    ...
    def __post_init__(self) -> None:
        for value, label in ((self.failed_route_digest, "failed_route_digest"), (self.failure_fingerprint, "failure_fingerprint"), (self.retained_candidate_digest, "retained_candidate_digest"), (self.delta_context_digest, "delta_context_digest")):
            require_digest(value, label)
        if not self.hypothesis.strip(): raise ValueError("escalation hypothesis is required")

    @property
    def digest(self) -> str:
        return canonical_digest({"failed_route_digest": self.failed_route_digest, "failure_fingerprint": self.failure_fingerprint, "hypothesis": self.hypothesis, "retained_candidate_digest": self.retained_candidate_digest, "delta_context_digest": self.delta_context_digest})
