...
from .explorer_successor_contracts import (
...
def _verify_packaged_phase2_bytes(generated: dict[str, bytes]) -> None:
    ...
    canonical_bytes_observed = (
        foundation.joinpath("canonical", "agents", "explorer.json").read_bytes()
    )
    if digest_bytes(canonical_bytes_observed) != BASE_CANONICAL_FILE_DIGEST:
        raise ValueError("packaged Phase 2 Explorer canonical bytes drifted")
