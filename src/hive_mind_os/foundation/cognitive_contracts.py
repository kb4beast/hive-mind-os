...
COGNITIVE_SCHEMA_NAMES = (
    ...
    "cognitive-failure-v1",
    "cognitive-home-v1",
    "cognitive-manifest-v1",
    "cognitive-note-v1",
    "cognitive-receipt-v1",
    "cognitive-result-v1",
    "cognitive-transaction-v1",
)


def load_cognitive_schema(name: str) -> dict[str, Any]:
    if name not in COGNITIVE_SCHEMA_NAMES:
        raise KeyError(f"unknown cognitive schema: {name}")
...
def validate_cognitive(name: str, document: Any) -> FoundationValidation:
    try:
        schema = load_cognitive_schema(name)
...
def validate_cognitive_catalog() -> FoundationValidation:
    issues: list[str] = []
    identifiers: set[str] = set()
    for name in COGNITIVE_SCHEMA_NAMES:
        try:
            schema = load_cognitive_schema(name)
