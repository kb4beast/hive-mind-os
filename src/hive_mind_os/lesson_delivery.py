...
class LessonDeliveryObligation:
 ...
 def __post_init__(self):
  if any(type(x) is not str or not x.strip() for x in (self.external_mission_id,self.origin_mission_token,self.subject_id,self.safe_draft_digest,self.local_artifact_path,self.delivery_idempotency_key)): raise ValueError("delivery identity is required")
  if self.upstream_path and (self.upstream_path.startswith("/") or ".." in self.upstream_path.split("/")): raise ValueError("invalid upstream path")
 @property
 ...
 def from_dict(cls,value):
  ...
  if set(value)!=fields: raise ValueError("closed delivery schema")
  return cls(**{**value,"obligation_state":ObligationState(value["obligation_state"])})
...
class DurableDeliveryLedger(DeliveryLedger):
 def __init__(self,path):
  super().__init__(); self._store=ContractStore(path,LessonDeliveryObligation.from_dict)
 def admit(self,item):
  key=item.subject_id+":"+item.external_mission_id
  old=self._store.get(key)
  if old and old.safe_draft_digest!=item.safe_draft_digest: raise ValueError("mission obligation already exists")
  result=super().admit(item); self._store.put(key,result); return result
 def get(self,subject_id,mission_id): return self._store.get(subject_id+":"+mission_id) or super().get(subject_id,mission_id)
