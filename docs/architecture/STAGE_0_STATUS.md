...
| Protected governance and CI | P07 independently reproduced active matching remote protection, two required approvals, CODEOWNERS review, stale-review dismissal, required CI, scans, SBOM, and provenance | The single-maintainer repository and administrator bypass mean rule activation alone does not prove an independent human approval |
...
- Stage 0 is not called release-ready while source evidence, licenses, authenticated
  identities, independent human approval, or later operational verification remain
  unresolved.
...
source, licensing, single-maintainer/admin-bypass, identity, durable-operation,
independent-verification, and production obligations. Implementation sequencing is owned by
