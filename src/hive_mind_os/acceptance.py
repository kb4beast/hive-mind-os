...
A prose acceptance criterion alone cannot prove that an arbitrary command is relevant.
An :class:`AcceptanceSpecification` therefore makes the command and expected process
outcome part of the accepted mission input.  Curator checks must reproduce that exact
specification; they cannot substitute a model-proposed command after the mission starts.
"""
...
class AcceptanceSpecification:
    ...
    ``criterion`` is the human-readable contract. ``argv`` and ``expected`` are the
    deterministic executable definition. Their canonical digest is recorded in the
    Curator seal, so a later command is objectively relevant only when it is byte-for-
    byte equivalent at the argv field boundary and has the same expected outcome.
    """
