...
class ValidationScope:
    ...
    def to_document(self) -> dict[str, Any]:
        return {"schema_version": 2, "tenant_digest": self.tenant_digest, "trust_domain_digest": self.trust_domain_digest, "adapter_digest": self.adapter_digest, "policy_digest": self.policy_digest, "external_state_digest": self.external_state_digest, "purpose": self.purpose.value, "scope_digest": self.digest}

    @classmethod
    def from_document(cls, value: Mapping[str, Any]) -> "ValidationScope":
        fields = {"schema_version", "tenant_digest", "trust_domain_digest", "adapter_digest", "policy_digest", "external_state_digest", "purpose", "scope_digest"}
        if set(value) != fields or value.get("schema_version") != 2: raise ValueError("validation scope has unknown shape")
        try: result = cls(value["tenant_digest"], value["trust_domain_digest"], value["adapter_digest"], value["policy_digest"], value["external_state_digest"], CheckPurpose(value["purpose"]))
        except (KeyError, TypeError, ValueError) as error: raise ValueError("validation scope is invalid") from error
        if value["scope_digest"] != result.digest: raise ValueError("validation scope digest mismatch")
        return result
...
def admit_cached_result(result: CachedTestResult | None, *, stored_scope: ValidationScope, requested_scope: ValidationScope, live_check: bool=False) -> InvalidationReport:
    if not isinstance(stored_scope, ValidationScope) or not isinstance(requested_scope, ValidationScope): return InvalidationReport(False,"validation scope is not authenticated","run-required-check")
    ...
    if live_check and (requested_scope.external_state_digest is None or stored_scope.external_state_digest is None): return InvalidationReport(False,"live-state freshness is unknown","run-required-check")
