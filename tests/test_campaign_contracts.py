...
def pkg(*, deps=(), target="src", req=("R01",), state=PackageState.READY, authority=D):
    a = AcceptanceBinding(
        "A01", "test", "receipt", "passed", "curator", ClaimLevel.RUNTIME
    )
    return WorkPackage(
        1,
        1,
...
def mission(*, packages=None, req=("R01",), target="src", parent=None, revision=1):
    return CampaignMission(
        ...
        target,
        req,
        (pkg(),) if packages is None else packages,
        D,
        CampaignState.READY,
        T,
        parent,
...
class TestCampaign(unittest.TestCase):
    def test_paths_coverage_cycle_and_authority_close(self):
        ...
        with self.assertRaises(CampaignContractError):
            mission(packages=(pkg(target="other"),))
        with self.assertRaises(CampaignContractError):
            mission(req=("R01", "R02"))
        with self.assertRaises(CampaignContractError):
            mission(packages=(pkg(deps=("PKG-two",)), pkg()))
        with self.assertRaises(CampaignContractError):
            mission(packages=(pkg(authority="sha256:" + "1" * 64),))
    ...
    def test_kernel_adapter_and_historical_gate(self):
        self.assertEqual(
            campaign_state_event(CampaignState.READY),
            ("mission.transition", {"status": "READY"}),
        )
        self.assertEqual(
            package_state_event(PackageState.VERIFYING),
            ("work.transition", {"status": "AWAITING_VERIFICATION"}),
        )
        raw = b'{"schema_version":0,"historical_inert_plan":true,"fixture_id":"N04-HISTORICAL-V0","candidate":"candidate/app.txt"}'
        self.assertEqual(
            parse_historical_inert_plan(raw, fixture_mode=True)["schema_version"], 0
        )
        with self.assertRaises(CampaignContractError):
            parse_historical_inert_plan(raw)
        root = Path(__file__).parent / "fixtures" / "campaign_contracts"
        self.assertEqual(
            parse_historical_inert_plan(
                (root / "historical-v0.json").read_bytes(), fixture_mode=True
            )["fixture_id"],
            "N04-HISTORICAL-V0",
        )
        with self.assertRaises(CampaignContractError):
            parse_historical_inert_plan(
                (root / "historical-v0-invalid-extra.json").read_bytes(),
                fixture_mode=True,
            )

    def test_successor_authenticated(self):
        p = mission()
        child = mission(parent=p.digest, revision=2)
        SuccessorContract(p, child)
        with self.assertRaises(CampaignContractError):
            SuccessorContract(p, mission(parent=p.digest, revision=1))
    ...
    def test_terminal_claim_consistency(self):
        ...
        bad = AcceptanceBinding(
            "A01", "test", "receipt", "failed", "curator", ClaimLevel.RUNTIME
        )
        with self.assertRaises(CampaignContractError):
            WorkPackage(
                1,
