...
class ConfiguredMissionBindingTests(unittest.TestCase):
    ...
    def test_wrong_tenant_blocks_before_resolver(self):
        called = []
        p = ConfiguredMissionBindingsProvider(
            self.descriptor(), lambda *_: called.append(True)
        )
        with self.assertRaises(MissionBindingError):
            p.resolve(
                {
                    "tenant_id": "other",
                    "repository_id": "repository",
                    "configuration_digest": self.descriptor().digest,
                },
